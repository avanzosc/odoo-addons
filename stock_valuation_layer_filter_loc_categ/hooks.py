...
def load_data_into_stock_valuation_layer(cr, registry):
    ...
    cr.execute(
        "update stock_valuation_layer "
        "set not_show_category_in_inventory_reports = (select product_category.not_show_in_inventory_reports "
    ...
    cr.execute(
        "update stock_valuation_layer "
        "set not_show_category_in_inventory_reports = (select stock_move.not_show_category_in_inventory_reports "
        "from stock_move "
        "where stock_move.id = stock_valuation_layer.stock_move_id) "
        "where stock_move_id is not null"
    )
    ...
    cr.execute(
        "update stock_valuation_layer "
        "set not_show_location_in_inventory_reports = (select stock_move.not_show_location_in_inventory_reports "
        "from stock_move "
        "where stock_move.id = stock_valuation_layer.stock_move_id) "
        "where stock_move_id is not null"
    )
