...
class StockValuationLayer(models.Model):
    ...
    location_id = fields.Many2one(
        string="Location", comodel_name="stock.location", readonly=True, index=True
    )
    ...
    def put_category_and_locations_info(self):
        ...
        if self.product_id and self.product_id.categ_id:
            ...
            show = categ.not_show_in_inventory_reports
            vals.update(
                {
    ...
    def put_not_show_in_inventory_reports_info(self):
        ...
        if (
            self.stock_move_id.location_id
            and self.stock_move_id.location_id.not_show_in_inventory_reports
        ):
