...
class StockMoveLine(models.Model):
    ...
    def _catch_values_to_create_machine(self):
        ...
        if self.product_id.default_code:
            name = (self.product_id.default_code if not name else
                    u"{} {}".format(name, self.product_id.default_code))
        if self.lot_name:
            name = (self.lot_name if not name else
                    u"{} {}".format(name, self.lot_name))
        vals = {
            "name": name,
