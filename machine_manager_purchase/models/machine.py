...
class Machine(models.Model):
    ...
    purchase_id = fields.Many2one(
        ...
        compute="_compute_purchase_info", store=True, copy=False
    )
    ...
    in_picking_id = fields.Many2one(
        string="In Picking", comodel_name="stock.picking",
        compute="_compute_purchase_info", store=True, copy=False
    )

    @api.depends("move_line_id", "move_line_id.move_id",
    ...
    def _compute_purchase_info(self):
        for machine in self:
            ...
            if pline.order_id.date_approve:
                ...
                purch_date = (
                    pline.order_id.date_approve.now().astimezone(
                        user_pytz).replace(tzinfo=None).date())
