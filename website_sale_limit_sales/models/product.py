...
class ProductBrand(models.Model):
    ...
    max_per_order = fields.Integer(
        string="Max brand products per order",
        help="Maximum qty of product from this brand that can"
             " be sold on each sale order")
    max_per_product_order = fields.Integer(
        string="Max from each product per order",
        help="Maximum qty from each product from this brand "
             "that can be sold on each sale order")
    limited_categories = fields.Many2many(
        string="Limited categories", comodel_name="product.category")
    limit_expiration_days = fields.Integer(
        string="Sales limitation expiration days",
        help="Expiration days for website sales limitation")
    product_category_ids = fields.Many2many(
        string="Related product categories", comodel_name="product.category",
