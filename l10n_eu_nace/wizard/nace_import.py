...
class NaceImport(models.TransientModel):
    ...
    def _download_nace(self, lang_code):
        ...
        pattern = re.compile(br'^.*<\?xml', re.DOTALL)
        content_fixed = re.sub(pattern, b'<?xml', res_request.content)
        if not re.match(br'<\?xml', content_fixed):
            raise exceptions.UserError(
                _('Downloaded file is not a valid XML file')
            )
