    ...
    def action_repair_lot_location(self):
        ...
        for lot in self:
            if lot.product_id.tracking == "serial":
                quant_internal = self.env["stock.quant"].search([
                    ("lot_id", "=", lot.id),
                    ("quantity", ">", 0)]).filtered(
                        lambda c: c.location_id.usage == "internal")
                quant_virtual = self.env["stock.quant"].search([
                    ("lot_id", "=", lot.id)]).filtered(
                        lambda c: c.location_id.usage != "internal")
                ...
                if not quant_internal and len(quant_virtual) > 1:
                    ...
                    if not last_quant:
                        last_quant = max(
                            quant_virtual, key=lambda x: x.create_date)
