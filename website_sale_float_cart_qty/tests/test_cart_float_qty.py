...
class TestSaleOrderState(HttpCase):

    def setUp(self):
        super().setUp()
        # Create a sale order in a non-draft state
        self.sale_order = self.env["sale.order"].create(
            {
                "partner_id": self.env.ref("base.res_partner_1").id,
                "state": "sale",  # Ensure state is not 'draft'
            }
        )

        # Create a product to be used in the test
        self.product = self.env["product.product"].create(
            {
                "name": "Test Product",
                "type": "consu",  # assuming 'consu' type is allowed to add to cart
    ...
    def test_sale_order_state_not_draft(self):
        """Test that modifying a sale order which is not in draft state raises an error"""

        # Mock the request object
        with patch("odoo.http.request") as mock_request:
            # Set up the mock request environment
            mock_request.env = self.env
            mock_request.session = {}  # Mock session if necessary

            # Attempt to perform an action that should fail if the order is not in draft state
            with self.assertRaises(UserError):
                # Simulate an action that modifies the sale order
                self.sale_order._cart_update(
                    product_id=self.product.id, line_id=False, add_qty=1, set_qty=False
                )
