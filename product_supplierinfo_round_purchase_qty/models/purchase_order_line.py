    ...
    def _round_line_qty(self, result):
        ...
        seller = line.product_id._select_seller(
            ...
            date=((line.order_id.date_order and
                   line.order_id.date_order.date()) or (
        ...
        if seller and seller.round_quantity_purchase:
            ...
            if rounded:
                message = _("THE QUANTITY TO BUY has been rounded to a"
                            " multiple of {}").format(round_qty)
                if ("warning" in result and result.get("warning", False) and
                        message not in result.get("warning").get("message")):
                    w = result.get("warning")
                    m = w.get("message")
                    new_message = u"{}\n{}".format(message, m)
                    result["warning"]["message"] = new_message
                else:
                    title = _(u"WARNING for {}").format(line.product_id.name)
