...
class EventEvent(models.Model):
    ...
    def fix_event_price_shared(self):
        events = self.filtered(
            lambda x: x.shared_price_event and x.task_id and x.sale_order_lines_ids
        )
        for event in events:
            ...
            for sale in sales:
                for line in sale.order_line.filtered(
                    ...
                    task_name = "{}: {}".format(sale.name, line.name)
                    if event.project_id.name == sale.name:
                        ...
                        line.write(
                            {"project_id": event.project_id.id, "task_id": new_task.id}
                        )
