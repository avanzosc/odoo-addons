...
class ReportStockPickingBatchXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        for line in objects:
            worksheet.write(
                n, 0, line.name if line.name else '')
            worksheet.write(
                n, 1, fields.Date.from_string(
                    line.entry_date).strftime(
                        '%d-%m-%Y') if line.entry_date else '')
            worksheet.write(
                n, 2, fields.Date.from_string(
                    line.cleaned_date).strftime(
                        '%d-%m-%Y') if line.cleaned_date else '')
            worksheet.write(
                n, 3, line.warehouse_id.name if line.warehouse_id else '')
            ...
            worksheet.write(n, 7, round(
                line.cancellation_percentage, 2), two_decimal_format)
            worksheet.write(n, 8, round(line.density, 2), two_decimal_format)
            worksheet.write(n, 9, round(
                line.growth_speed, 2), two_decimal_format)
            worksheet.write(n, 10, line.feed, int_format)
            worksheet.write(n, 11, line.meat_kilos, int_format)
            worksheet.write(n, 12, round(
                line.output_amount, 2), two_decimal_format)
            worksheet.write(n, 13, line.consume_feed, int_format)
            worksheet.write(n, 14, round(
                line.output_feed_amount, 2), two_decimal_format)
            worksheet.write(
                n, 15, line.feed_family.name if line.feed_family else '')
            worksheet.write(n, 16, round(
                line.output_medicine_amount, 2), two_decimal_format)
            worksheet.write(n, 17, round(
                line.average_age, 2), two_decimal_format)
            worksheet.write(n, 18, line.farm_day, int_format)
            worksheet.write(n, 19, round(
                line.average_weight, 3), three_decimal_format)
            worksheet.write(n, 20, round(
                line.conversion, 3), three_decimal_format)
            worksheet.write(n, 21, round(
                line.dif_weight, 3), three_decimal_format)
            worksheet.write(n, 22, round(
                line.liquidation_amount, 2), two_decimal_format)
            worksheet.write(n, 23, round(
                line.chick_liquidation, 8), eight_decimal_format)
            worksheet.write(n, 24, round(
                line.liquidation_area, 8), eight_decimal_format)
            worksheet.write(n, 25, round(
                line.cost_kilo, 3), three_decimal_format)
            n = n + 1
        worksheet.write(n, 5, sum(
            objects.mapped("chick_entry_qty")), int_format)
        ...
        worksheet.write(n, 12, round(sum(objects.mapped(
            "output_amount")), 2), two_decimal_format)
        ...
        worksheet.write(n, 16, round(sum(objects.mapped(
        ...
        worksheet.write(n, 25, round(
            sum(objects.mapped("cost_kilo"))/(n-1), 3), three_decimal_format)
