...
class MotherLineageRelation(models.Model):
    ...
    entry_date = fields.Date(
        string="Entry Date",
        related="breeding_id.entry_date",
        store=True)
    cleaned_date = fields.Date(
        string="Cleaned Date",
        related="breeding_id.cleaned_date",
        store=True)
    ...
    density = fields.Float(
        string="Density",
        related="breeding_id.density",
        store=True)
    growth_speed = fields.Float(
        string="Growth Speed",
        related="breeding_id.growth_speed",
        store=True)
    feed = fields.Integer(
        string="Feep",
        related="breeding_id.feed",
        store=True)
    meat_kilos = fields.Float(
        string="Meat Kilos",
        compute="_compute_meat_kilos",
        store=True)
    ...
    average_age = fields.Float(
        string="Average Age",
        related="breeding_id.average_age",
        store=True)
    farm_day = fields.Integer(
        string="Farm Day",
        related="breeding_id.farm_day",
        store=True)
    average_weight = fields.Float(
        string="Average Weight",
        related="breeding_id.average_weight",
        store=True)
    conversion = fields.Float(
        string="Conversion",
        related="breeding_id.conversion",
        store=True)
    dif_weight = fields.Float(
        string="Dif.",
        related="breeding_id.dif_weight",
        store=True)
    ...
    chick_liquidation = fields.Float(
        ...
        related="breeding_id.chick_liquidation",
        store=True)
    liquidation_area = fields.Float(
        string="Liquidation Area",
        related="breeding_id.liquidation_area",
        store=True)
    cost_kilo = fields.Float(
        string="Cost Kilo",
        related="breeding_id.cost_kilo",
        store=True)
    liquidated = fields.Boolean(
        string="Liquidated",
        related="breeding_id.liquidated",
        store=True)
    billed = fields.Boolean(
        string="Billed",
        related="breeding_id.billed",
        store=True)
    closed = fields.Boolean(
        string="Closed",
        related="breeding_id.closed",
        store=True)
    output_amount_days = fields.Float(
        string="Output Amount Days",
        compute="_compute_output_amount_days",
        store=True)
    ...
    medicine_amount = fields.Float(
        string="Medicine Amount",
        compute="_compute_medicine_amount",
        store=True)
    warehouse_area = fields.Float(
        string="Warehouse Area",
        compute="_compute_warehouse_area",
        store=True)
    ...
    def _compute_age_output(self):
        for line in self:
            ...
            if line.breeding_id:
                age_output = (
                    line.breeding_id.age_output * line.percentage) / 100
    ...
    def _compute_warehouse_area(self):
        for line in self:
            ...
            if line.breeding_id:
                warehouse_area = (
                    line.breeding_id.warehouse_area * line.percentage) / 100
    ...
    def _compute_output_feed_amount(self):
        for line in self:
            ...
            if line.breeding_id:
                qty = (
                    line.breeding_id.output_feed_amount * line.percentage
                    ) / 100
    ...
    def _compute_meat_kilos(self):
        for line in self:
            ...
            if line.breeding_id:
                meat = (
                    line.breeding_id.meat_kilos * line.percentage) / 100
    ...
    def _compute_output_amount_days(self):
        for line in self:
            ...
            if line.breeding_id:
                amount = (
                    line.breeding_id.output_amount_days * line.percentage
                    ) / 100
    ...
    def _compute_consume_feed(self):
        for line in self:
            ...
            if line.breeding_id:
                feed = (
                    line.breeding_id.consume_feed * line.percentage) / 100
    ...
    def _compute_liquidation_amount(self):
        for line in self:
            ...
            if line.breeding_id:
                amount = (
                    line.breeding_id.liquidation_amount * line.percentage
                    ) / 100
    ...
    def read_group(
            ...
            lazy=True):
        result = super(MotherLineageRelation, self).read_group(
