...
class EventAttendanceReport(models.Model):
    ...
    session_date_time_type = fields.Char(
        string="Session date&Time type", readonly=1)

    def _select_event_attendace_report(self):
        select = super(
            EventAttendanceReport, self)._select_event_attendace_report()
        return "{}, {}".format(
            select, "p.id as time_type_id, "
    ...
    def _from_event_attendace_report(self):
        ...
        return "{} {}".format(
            super_from,
            ' inner join project_time_type p on p.id = t.time_type_id ')
