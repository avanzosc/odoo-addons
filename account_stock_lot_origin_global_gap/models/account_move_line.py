...
class AccountMoveLine(models.Model):
    ...
    lot_id = fields.Many2one(
        string="Lot", comodel_name="stock.lot",
        compute="_compute_country_global_gap")
    lot_country_id = fields.Many2one(
        string="Lot Origin", comodel_name="res.country",
        compute="_compute_country_global_gap")
    ...
    def _compute_country_global_gap(self):
        for line in self:
            ...
            for stock_move in stock_moves:
                for move_line in stock_move.move_line_ids.filtered(
                    ...
                    if move_line.lot_country_gloval_gap_of:
                        ...
                        lot_global_gap_to_print = (
                            move_line.lot_global_gap_to_print)
                        lot_country_gloval_gap_of = (
                            move_line.lot_country_gloval_gap_of)
                    else:
            ...
            line.lot_country_id = (
                lot_country.id if lot_country else lot_country)
