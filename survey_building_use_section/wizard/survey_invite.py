...
class SurveyInvite(models.TransientModel):
    ...
    partner_id = fields.Many2one(
        string="Recipient", comodel_name="res.partner"
    )
    ...
    building_activation_date = fields.Date(
        string="building activation date"
    )
    ...
    def _onchange_building_to_inspect_id(self):
        ...
        if (self.building_to_inspect_id and not
    ...
    def _prepare_building_answers(self, partners, emails):
                ...
                for partner_done in partners_done:
                    answers |= next(existing_answer for existing_answer in
                                    existing_answers.sorted(
                                        lambda answer: answer.create_date,
                                        reverse=True)
        ...
        for new_partner in partners - partners_done:
            answers |= self.survey_id.with_context(
                building=self.building_to_inspect_id)._create_answer(
                    partner=new_partner, check_attempts=False,
