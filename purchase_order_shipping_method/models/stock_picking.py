...
class StockPicking(models.Model):
    ...
    carrier_id = fields.Many2one(string="Shipping Method")
    ...
    def _compute_transport_price(self):
        for picking in self:
            picking.transport_price = picking.shipping_cost * picking.total_done_qty

    def action_invoice_trasport_lines(self):
        if (
            self.shipping_cost > 0
            and (self.carrier_id is not False)
            and (self.state == "done" and self.transporter_id)
        ):
