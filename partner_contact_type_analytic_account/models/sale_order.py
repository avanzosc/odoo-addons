...
class SaleOrder(models.Model):
    ...
    contact_type_id = fields.Many2one(
        ...
        related="partner_invoice_id.contact_type_id",
        store=True,
    )
    ...
    @api.depends(
        "partner_invoice_id",
        "partner_invoice_id.contact_type_id",
        "contact_type_id.analytic_account_id",
    )
    def _compute_analytic_account_id(self):
        for sale in self:
            if (
                sale.partner_invoice_id.contact_type_id
                and sale.partner_invoice_id.contact_type_id.with_company(
                    sale.company_id.id
                ).analytic_account_id
            ):
