...
class SaleOrderLine(models.Model):
    ...
    def _compute_qty_delivered(self):
        ...
        for line in self:
            if line.qty_delivered_method == "stock_move" and (
                line.order_id.update_line_qty
            ) and line.return_qty:
                line.product_uom_qty = line.qty_delivered
            if len(
                line.order_id.picking_ids
            ) > 1 and line.order_id.update_line_qty and all(
                [c.state == "done" for c in line.order_id.picking_ids]
            ) and all(
                [c.product_uom_qty == c.qty_delivered for c in (
