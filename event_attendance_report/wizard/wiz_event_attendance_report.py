...
class WizEventAttendanceReport(models.TransientModel):
    ...
    allowed_customer_ids = fields.Many2many(
        string="Allowed customers", comodel_name="res.partner",
        relation='rel_wiz_event_attendance_report_customer',
        column1='wiz_id', column2='allowed_customer_id')
    allowed_event_ids = fields.Many2many(
        string="Allowed events", comodel_name="event.event",
        relation='rel_wiz_event_attendance_report_event',
        column1='wiz_id', column2='allowed_event_id')
    allowed_student_ids = fields.Many2many(
        string="Allowed students", comodel_name="res.partner",
        relation='rel_wiz_event_attendance_report_student',
        column1='wiz_id', column2='allowed_student_id')
    ...
    def default_get(self, fields_list):
        res = super(WizEventAttendanceReport, self).default_get(
            fields_list)
        lines = self.env['event.attendance.report'].search([])
        if lines:
            res.update(
                {"allowed_customer_ids":
                 [(6, 0, lines.mapped('customer_id').ids)],
                 "allowed_event_ids": [(6, 0, lines.mapped('event_id').ids)],
                 "allowed_student_ids":
                 [(6, 0, lines.mapped('student_id').ids)]})
        return res
    ...
    def filter_lines(self, lines):
        if self.customer_id:
            lines = lines.filtered(
                lambda x: x.customer_id.id == self.customer_id.id)
        if self.event_id:
            lines = lines.filtered(
                lambda x: x.event_id.id == self.event_id.id)
        if self.student_id:
            lines = lines.filtered(
                lambda x: x.student_id.id == self.student_id.id)
        if self.date_begin:
            lines = lines.filtered(
                lambda x: x.session_date >= self.date_begin)
        if self.date_end:
            lines = lines.filtered(
                lambda x: x.session_date >= self.date_end)
        return lines
    ...
    def action_print(self):
        self.ensure_one()
        lines_to_print = self.set_allowed_data()
...
class WizAttendanceActivityReportLine(models.TransientModel):
    ...
    wiz_id = fields.Many2one(
        string="Wizard",
        comodel_name="wiz.event.attendance.report")
    event_id = fields.Many2one(
        string="Event", comodel_name="event.event")
    customer_id = fields.Many2one(
        string="Customer", comodel_name="res.partner")
