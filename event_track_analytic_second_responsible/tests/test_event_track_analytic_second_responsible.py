...
class TestEventTrackAnalyticSecondResponsible(common.SavepointCase):
    ...
    def setUpClass(cls):
        ...
        vals = {
            "name": "User event track analytic second responsible",
            "login": "uetasrt@avanzosc.es",
        }
        user = cls.env["res.users"].create(vals)
        vals = {
            "name": "employee event event track analyticsecondresponsible",
            "user_id": user.id,
        }
        cls.env["hr.employee"].create(vals)
        vals = {
            "name": "User2 event track analytic second responsible",
            "login": "u2etasrt@avanzosc.es",
        }
        user2 = cls.env["res.users"].create(vals)
        vals = {
            "name": "employee2 event track analyticsecondresponsible",
            "user_id": user2.id,
        }
        cls.env["hr.employee"].create(vals)
        ...
        cond = [("is_done", "=", True)]
        cls.track_done_state = cls.env["event.track.stage"].search(cond, limit=1)

    def test_event_track_analytic_second_reponsible(self):
        self.event.track_ids[0].stage_id = self.track_done_state.id
        self.assertEqual(len(self.event.track_ids[0].account_analytic_line_ids), 2)
