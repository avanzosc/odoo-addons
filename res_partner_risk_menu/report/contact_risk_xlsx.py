    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        if len(objects) != 1:
            raise ValidationError(
                _("The report cant be taken from a single contact."))
        ...
        delivery_pending_sales = sales.filtered(lambda c: sum(c.order_line.mapped("qty_to_deliver")) > 0)
        worksheet.write(n, 5, sum(delivery_pending_sales.mapped("amount_total")) if delivery_pending_sales else '', table_second_header)
        for sale in delivery_pending_sales:
            n += 1
            m = 1
            worksheet.write(n, m, fields.Date.from_string(sale.date_order).strftime('%d-%m-%Y') if sale.date_order else '')
            m += 1
        ...
        domain = objects.with_context(open_risk_field="risk_sale_order").open_risk_pivot_info()['domain']
        s = self.env["sale.order.line"].search(domain)
        sales = []
        worksheet.write(n, 5, sum(s.mapped("risk_amount")) if s else '', table_second_header)
        for sale in s:
            if sale.order_id not in sales:
                risk_amount = sum(s.filtered(lambda c: c.order_id == sale.order_id).mapped("risk_amount"))
                if risk_amount != 0:
                    ...
                    worksheet.write(n, m, fields.Date.from_string(sale.date_order).strftime('%d-%m-%Y') if sale.date_order else '')
                    m += 1
                    worksheet.write(n, m, sale.order_id.name if sale.order_id.name else '')
                    m += 3
                    worksheet.write(n, m, risk_amount, two_decimal_format)
        domain = objects.with_context(open_risk_field="risk_invoice_draft").open_risk_pivot_info()['domain']
        ...
        worksheet.write(n, 5, sum(invoices.mapped("amount_residual")) if invoices else '', table_second_header)
        for invoice in invoices:
            if invoice.move_id not in inv:
                inv.append(invoice.move_id)
                n += 1
                m = 1
                worksheet.write(n, m, fields.Date.from_string(invoice.invoice_date).strftime('%d-%m-%Y') if invoice.invoice_date else '')
                m += 2
                ...
                m += 1
                worksheet.write(n, m, fields.Date.from_string(invoice.move_id.invoice_date_due).strftime('%d-%m-%Y') if invoice.move_id.invoice_date_due else '')
        n += 1
        for i in range(0, 10):
            worksheet.write(n, i, '', table_second_header)
        domain = objects.with_context(open_risk_field='risk_invoice_open').open_risk_pivot_info()['domain']
        invoices = self.env["account.move.line"].search(domain)
        inv = []
        ...
        for i in range(0, 10):
            worksheet.write(n, i, '', table_second_header)
        domain = objects.with_context(open_risk_field='risk_invoice_unpaid').open_risk_pivot_info()['domain']
        invoices = self.env["account.move.line"].search(domain)
        inv = []
        worksheet.write(n, 0, _('Invoices/Balance Open'), table_second_header)
        worksheet.write(n, 5, sum(invoices.mapped("amount_residual")) if invoices else '', table_second_header)
        ...
        for i in range(0, 10):
            worksheet.write(n, i, '', table_second_header)
        domain = objects.with_context(open_risk_field='risk_account_amount').open_risk_pivot_info()['domain']
        invoices = self.env["account.move.line"].search(domain)
        inv = []
        ...
        for i in range(0, 10):
            worksheet.write(n, i, '', table_second_header)
        domain = objects.with_context(open_risk_field='risk_account_amount_unpaid').open_risk_pivot_info()['domain']
        invoices = self.env["account.move.line"].search(domain)
        inv = []
