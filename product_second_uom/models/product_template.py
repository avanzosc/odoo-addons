...
class ProductTemplate(models.Model):
    ...
    factor = fields.Float(string="Conversion Factor", default=1, digits="Factor")
    factor_inverse = fields.Float(
        string="Conversion Factor Inverse",
        compute="_compute_factor_inverse",
        digits="Factor",
    )
    weight_second_uom = fields.Float(
        string="Weight Second UOM", compute="_compute_weight_second_uom", store=True
    )
    qty_available_second_uom = fields.Float(
        string="Available in Second UOM",
        compute="_compute_qty_available_second_uom",
        store=True,
    )
    virtual_available_second_uom = fields.Float(
        string="Virtual Second UOM",
        compute="_compute_virtual_available_second_uom",
        store=True,
    )
    ...
    def _compute_virtual_available_second_uom(self):
        for product in self:
            product.virtual_available_second_uom = (
                product.virtual_available * product.factor
            )
