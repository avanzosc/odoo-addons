...
class PurchaseOrder(models.Model):
    ...
    price_total_eur = fields.Monetary(
        string="Total EUR", compute="_compute_price_total_eur", store=True, copy=False
    )
    price_subtotal_eur = fields.Monetary(
        string="SubTotal EUR",
        compute="_compute_price_total_eur",
        store=True,
        copy=False,
    )

    @api.depends(
        "order_line", "order_line.price_total_eur", "order_line.price_subtotal_eur"
    )
