...
class StockPicking(models.Model):
    ...
    def button_validate(self):
        ...
        for picking in self:
            if not picking.company_id.tolvasa and picking.sale_id and (
                ...
                if purchase_picking:
                    ...
                    for line in picking.move_line_ids_without_package.filtered(
                        lambda c: (
                            c.lot_id)):
                        if not purchase_picking.move_line_ids_without_package.filtered(lambda c: c.product_id == line.product_id and c.lot_id == line.lot_id):
                            purchase_line = purchase_picking.move_line_ids_without_package.filtered(lambda c: c.product_id == line.product_id and not c.lot_id)
