...
class EventEvent(models.Model):
    ...
    def write(self, values):
        if (
            ("lang_id" in values and values.get("lang_id", False))
            or ("level_id" in values and values.get("level_id", False))
            or ("date_begin" in values and values.get("date_begin", False))
        ):
            name = ""
            ...
            if "date_begin" in values and values.get("date_begin", False):
                date_begin = fields.Datetime.from_string(values.get("date_begin"))
                name = (
                    date_begin.year
                    if not name
                    else "{}-{}".format(name, date_begin.year)
                )
            else:
                name = (
                    self.date_begin.year
                    if not name
                    else "{}-{}".format(name, self.date_begin.year)
                )
            if "level_id" in values and values.get("level_id", False):
                level = self.env["hr.skill.level"].browse(values.get("level_id"))
                name = level.name if not name else "{}-{}".format(name, level.name)
            else:
                if self.level_id:
                    name = (
                        self.level_id.name
