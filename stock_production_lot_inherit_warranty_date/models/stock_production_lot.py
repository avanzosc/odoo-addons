        ...
        for lot in self:
            if lot.of_lots_ids:
                ...
                if lots:
                    ...
                    for lot in lots:
                        lot.with_context(
                            mrp_children_lot=True)._onchange_expiration_date()
        return result

    def write(self, vals):
        result = super(StockProductionLot, self).write(vals)
        if ("removal_date" in vals or "use_date" in vals or
                "alert_date" in vals):
            for lot in self:
                if lot.of_lots_ids:
                    lots = lot.of_lots_ids.mapped("lot_id")
                    ...
                    if vals.get("removal_date"):
                        childrens_lot_vals["removal_date"] = vals.get(
                            "removal_date")
                    if vals.get("use_date"):
                        childrens_lot_vals["use_date"] = vals.get(
                            "use_date")
                    if vals.get("alert_date"):
                        childrens_lot_vals["alert_date"] = vals.get(
                            "alert_date")
