...
class StockPicking(models.Model):
    ...
    def _compute_packages_volume(self):
        for picking in self:
            picking.packages_volume = (
                sum(picking.quant_package_ids.mapped("volume")))
    ...
    def _compute_volume_uom_name(self):
        for package in self:
            if package.quant_package_ids:
                package.volume_uom_name = (
                    package.quant_package_ids[0].volume_uom_name)
