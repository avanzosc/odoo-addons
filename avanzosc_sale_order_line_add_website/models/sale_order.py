...
class SaleOrderLine(models.Model):
    ...
    def _update_plan(self, product_name, website_name):
        ...
        name = u'{}-{}'.format(product_name, website_name)
    ...
    def _onchange_website_id(self):
        ...
        if self.website_id and self.product_id:
            ...
            self.analytics_id = self._update_plan(product_name, website_name)

    @api.multi
    def _create_analytic_account_and_instance(
            self, product_name, website_name, plan):
        parent_id = self.env['res.company'].search([])[0]
        self._get_analytic_account(
            product_name, parent_id.parent_product, plan)
        self._get_analytic_account(
            website_name, parent_id.parent_website, plan)
