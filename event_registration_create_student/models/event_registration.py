...
class EventRegistration(models.Model):
    ...
    create_student_check = fields.Boolean(related="event_id.create_student_check")
    ...
    def create_registration_student(self):
        ...
        for registration in self:
            ...
            if (
                student_partner
                and registration.generate_student_email_check
            ...
            if not student_user and registration.create_student_user_check:
                student_user = registration.action_create_student_user(student_partner)

        return True

    def action_create_student_partner(self, force_create=False):
        if not self.partner_id:
            raise ValidationError(_("The ticket reserved by is not specified!"))
        ...
        if not force_create and not self.create_student_check:
            ...
            if not self.name:
                raise ValidationError(
                    _("You must first fill the participant data! " "(Name, email...)")
                )

            if not force_create:
                # Check if user already exists with the set email field in
                # event.registration
                attendee_user = self.create_get_user({"login": self.email}, False)

                if attendee_user and attendee_user.partner_id == booked_by:
                    raise ValidationError(
                        _(
                            "Notice that you are confirming an attendee that  "
                            "coincides with the Reserved by partner email."
                        )
                    )

                student_user = attendee_user
                student_partner = attendee_user.partner_id if student_user else None
            else:
        ...
        self.write({"student_id": student_partner.id})
    ...
    def action_create_student_user(self, student_partner=None):
        ...
        if not student_partner.email:
            if not self.email:
                raise ValidationError(
                    _(
                        "You must set/generate an email for the attendee"
    ...
    def action_generate_student_email(self, student_partner):
        ...
        student_partner.write({"email": student_email})
        self.write({"email": student_email})
