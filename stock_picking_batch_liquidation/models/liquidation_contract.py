...
class LiquidationContract(models.Model):
    ...
    chicken_load = fields.Float(
        string="Chicken Load",
        digits="Chicken Load Decimal Precision")
    correction_factor = fields.Float(
        string="Correction Factor")
    initial_feed = fields.Integer(
        string="Initial FEEP")
    final_feed = fields.Integer(
        string="Final FEEP")
    feed_ratio = fields.Float(
        string="FEEP Ratio",
        digits="Feep Decimal Precision")
    feed_inital_price = fields.Float(
        string="FEEP Initial Price",
        digits="Feep Decimal Precision")
    feed_rate_ids = fields.One2many(
        string="FEEP Rate",
        comodel_name="feed.rate",
    ...
    def action_create_feed_rate(self):
        ...
        if not self.initial_feed or not (
            self.final_feed) or not self.feed_ratio or not (
                self.feed_inital_price):
            raise ValidationError(
                _("some of the data required for the calculation of " +
