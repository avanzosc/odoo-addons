...
class StockPickingBatch(models.Model):
    ...
    def _compute_density(self):
        for batch in self:
            if batch.warehouse_id and batch.warehouse_id.farm_area != 0:
                batch.density = batch.chick_units / batch.warehouse_id.farm_area

    @api.depends("stage_id")
    def _compute_closed(self):
        for batch in self:
            try:
                closed = self.env.ref("stock_warehouse_farm.batch_stage3")
    ...
    def _compute_billed(self):
        for batch in self:
            try:
                billed = self.env.ref("stock_picking_batch_breeding.batch_stage6")
    ...
    def _compute_cancellation_area(self):
        for line in self:
            ...
            if line.warehouse_id.farm_area != 0:
                line.cancellation_area = (line.chick_units - line.output_units) / (
    ...
    def _compute_output_area(self):
        for line in self:
            line.output_area = 0
            if line.warehouse_id.farm_area != 0:
                line.output_area = line.output_units / line.warehouse_id.farm_area
    ...
    def _compute_entry_area(self):
        for line in self:
            line.entry_area = 0
            if line.warehouse_id.farm_area != 0:
                line.entry_area = line.chick_units / line.warehouse_id.farm_area
