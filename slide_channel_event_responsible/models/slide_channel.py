...
class SlideChannel(models.Model):
    ...
    def update_event_reponsible_in_slide_channel(self, event, responsible, date):
        ...
        if not date:
            if not slide_channel:
                self.create_responsible_in_slide_channel(event, responsible.partner_id)
            else:
        ...
        if date and slide_channel:
            events = self.event_ids.filtered(
                lambda x: x.id != event.id
                and not x.stage_id.pipe_end
                and (x.user_id and x.user_id == responsible)
                or (x.main_responsible_id and x.main_responsible_id == responsible)
                or (x.second_responsible_id == responsible)
            )
            if not events or (events and len(events) == 1) and event == event:
