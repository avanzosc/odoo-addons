...
class EventEvent(models.Model):
    ...
    def write(self, values):
        ...
        if "stage_id" in values and values.get("stage_id", 99) == confirm_stage.id:
            self.update_responsibles_in_slide_channel()
        if "stage_id" in values and values.get("stage_id", 99) == cancel_stage.id:
            self.update_responsibles_in_slide_channel(fields.Date.context_today(self))
        if "stage_id" in values and values.get("stage_id", 99) == done_stage.id:
            self.update_responsibles_in_slide_channel(self.date_end.date())
        return result
