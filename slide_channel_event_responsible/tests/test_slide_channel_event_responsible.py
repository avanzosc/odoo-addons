...
class TestSlideChannelEventResponsible(common.SavepointCase):
    ...
    def setUpClass(cls):
        ...
        vals = {
            "name": "User 1 for slide channel event_responsible",
            "login": "login1@avanzosc.es",
        }
        cls.user1 = cls.user_obj.create(vals)
        vals = {
            "name": "User 2 for slide channel event_responsible",
            "login": "login2@avanzosc.es",
        }
        cls.user2 = cls.user_obj.create(vals)
        vals = {
            "name": "User 3 for slide channel event_responsible",
    ...
    def test_slide_channel_event_responsible(self):
        ...
        cond = [
            ("channel_id", "=", self.slide_channel.id),
            (
                "partner_id",
                "in",
                (
                    self.user1.partner_id.id,
