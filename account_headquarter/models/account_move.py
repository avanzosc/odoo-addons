        ...
        for account_move in self:
            if account_move.headquarter_id:
                lines = account_move.invoice_line_ids.filtered(
                    lambda x: x.account_id and not x.exclude_from_invoice_tab
                )
                for line in lines:
                    ...
                    if not without_headquarter_control and not line.headquarter_id:
                        raise UserError(
                            _(
                                "The line with accounting account: {}, need to "
                                "define their Headquarter."
                            ).format(line.account_id.name)
                        )
                if lines:
                    lines2 = lines.filtered(
                        lambda x: x.headquarter_id and not x.analytic_account_id
                    )
                    for line in lines2:
                        raise UserError(
                            _(
                                "The line with accounting account: {}, need to "
                                "define their analytic account."
                            ).format(line.account_id.name)
                        )
                lines = account_move.invoice_line_ids.filtered(
