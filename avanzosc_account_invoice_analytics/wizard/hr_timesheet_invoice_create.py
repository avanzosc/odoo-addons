    ...
    def invoice_cost_create(self, cr, uid, ids, data={}, context=None):
        ...
        for account in analytic_account_obj.browse(cr, uid, account_ids, context=context):
            ...
            if partner.supplier and not partner.customer:
                curr_invoice['type'] = 'in_invoice'
                curr_invoice['journal_id'] = journal_obj.search(cr, uid, [('type','=','purchase')])[0]
                curr_invoice['account_id'] = partner.property_account_payable.id
            ...
            cr.execute("SELECT product_id, to_invoice, unit_amount, product_uom_id " \
                    "FROM account_analytic_line as line " \
                    "WHERE account_id = %s " \
                    "AND id IN %s AND to_invoice IS NOT NULL " \
                    "AND move_id IS NULL" , (account.id, tuple(ids),))
            ...
            for product_id, factor_id, qty, uom in cr.fetchall():
                ...
                if account.pricelist_id:
                    ...
                    if partner.supplier and not partner.customer:
                        pl = partner.property_product_pricelist_purchase.id
                ...
                account_id = product.product_tmpl_id.property_account_expense.id or product.categ_id.property_account_expense_categ.id
                if not account_id:
...
class hr_timesheet_invoice_create(osv.osv_memory):
    ...
    def do_create(self, cr, uid, ids, context=None):
        ...
        type = inv_obj.browse(cr, uid, invs)[0].type
        if type == 'in_invoice':
        ...
        if type == 'in_invoice':
            act_win['view_id'] = 'account.invoice_supplier_form'
        act_win['domain'] = [('id','in',invs),('type','=',type)]
