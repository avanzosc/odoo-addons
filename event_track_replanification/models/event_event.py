...
class EventEvent(models.Model):
    ...
    replan_date_begin = fields.Date(string="Start Date", copy=False)
    replan_date_end = fields.Date(string="End Date", copy=False)
    ...
    def button_replan_sessions(self):
        ...
        if sessions:
            sessions = sessions.filtered(lambda x: not x.stage_id.is_cancel)
        for session in sessions:
            timezone = pytz.timezone(self.date_tz or "UTC")
            session_date = session.date.replace(tzinfo=pytz.timezone("UTC")).astimezone(
                timezone
            )
            session_date = session_date.date()
            if (
                session_date >= self.replan_date_begin
                and session_date <= self.replan_date_end
            ):
        ...
        self.with_context(
            ...
            rep_calendar=self.replan_resource_calendar_id,
        )._create_tracks()
        n = 1
