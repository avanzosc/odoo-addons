...
class CrmLead(models.Model):
    ...
    repair_product_id = fields.Many2one(
        string="Product to repair", comodel_name="product.product",
        copy=False)
    repair_product_qty = fields.Float(
        string="Quantity to repair", digits="Product Unit of Measure",
        copy=False)
    repair_lot_id = fields.Many2one(
        string="Lot for repair", comodel_name="stock.production.lot",
        copy=False)
    repair_order_ids = fields.One2many(
        string="Repairs", comodel_name="repair.order", copy=False,
        inverse_name="crm_lead_id")
    count_repair_orders = fields.Integer(
        string="Num. Repairs", store=True, copy=False,
    ...
    def action_create_repair_order(self):
        if not self.partner_id:
            raise ValidationError(
                _("You must enter the customer."))
        if not self.repair_product_id:
            raise ValidationError(
                _("You must enter the product to repair."))
