...
class PurchaseOrder(models.Model):
    ...
    loading_date = fields.Date(string="Date Loading", index=True)
    estimated_payment_date = fields.Date(
        string="Estimated date payment ",
        index=True,
        store=True,
    ...
    def _compute_estimated_payment_date(self):
        for purchase in self.filtered(
            ...
            date = (
                purchase.loading_date
                if purchase.loading_date
                else purchase.date_planned.date()
            )
            line = purchase.payment_term_id.line_ids[0]
            if line.nb_days:
                estimated_payment_date = date + relativedelta(days=line.nb_days - 1)
