...
class EventRegistration(models.Model):
    ...
    order_status = fields.Selection(
        string="Order status", store=True, related="sale_order_id.state"
    )
    event_ticket_product_id = fields.Many2one(
        string="Event ticket product",
        comodel_name="product.product",
        related="event_ticket_id.product_id",
        store=True,
    )
    event_ticket_price = fields.Float(
        string="Event ticket price",
    ...
    def _compute_with_distinct_product(self):
        for record in self:
            ...
            if (
                ...
                and record.sale_line_product_id
                and record.event_ticket_product_id != record.sale_line_product_id
            ):
                distinct = True
    ...
    def _compute_with_distinct_price(self):
        for record in self:
            distinct = False
            if (
                record.event_ticket_price
                and record.sale_line_price
                and record.event_ticket_price != record.sale_line_price
            ):
                distinct = True
    ...
    def _compute_event_partner_bank_acc(self):
        for record in self:
            record.partner_bank_acc_id = self.env["res.partner.bank"].search(
                [("id", "in", record.partner_id.bank_ids.ids)], order="id desc", limit=1
            )
