...
class StockProductionLot(models.Model):
    ...
    of_lots_ids = fields.Many2many(
        string="OF Consumed products with lot", comodel_name="stock.move.line",
        ...
        for lot in self:
            ...
            if production:
                ...
                for component in components:
                    for line in component.move_line_ids.filtered(
                            lambda z: z.lot_id):
                        if line not in move_lines:
                            move_lines += line
            lot.of_lots_ids = [(6, 0, [])] if not move_lines else [
                (6, 0, move_lines.ids)]
