...
class ResPartner(models.Model):
    ...
    child_medical_record = fields.Integer(string="Child Medical Record Number")
    adult_medical_record = fields.Integer(string="Adult Medical Record Number")
    ...
    def _compute_last_child_number(self):
        contacts = self.env["res.partner"].search([("child_medical_record", "!=", 0)])
        if contacts:
            self.last_child_number = max(
                contacts, key=lambda x: (x.child_medical_record)
            ).child_medical_record
        else:
            self.last_child_number = 0

    @api.depends("adult_medical_record")
    def _compute_last_adult_number(self):
        contacts = self.env["res.partner"].search([("adult_medical_record", "!=", 0)])
        if contacts:
            self.last_adult_number = max(
                contacts, key=lambda x: (x.adult_medical_record)
            ).adult_medical_record
        else:
