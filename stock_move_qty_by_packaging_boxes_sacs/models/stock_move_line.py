...
class StockMoveLine(models.Model):
    ...
    def _get_palet_qty(self):
        return self.qty_done / (self.palet_id.qty * self.palet_id.palet_qty)
    ...
    def _onchange_palet_qty(self):
        if (
            ...
            line = self.product_id.packaging_ids.filtered(
                lambda x: x.palet_id and x.palet_id == self.palet_id
            )
            if line and len(line) == 1:
                self.palet_qty = self.palet_qty * line.palet_qty
