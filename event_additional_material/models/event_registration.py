...
class EventRegistration(models.Model):
    ...
    def put_in_sale_order_additional_material(self):
        for registration in self.filtered(
            lambda x: x.event_id.add_mat_automatically
            and x.sale_order_id
            and x.state == "open"
        ):
    ...
    def update_sale_line_from_additional_material(self, sale_line, material):
        product_uom_qty = self.calculate_additional_material_qty(material)
        vals = {"product_uom_qty": product_uom_qty, "price_unit": material.price_unit}
    ...
    def calculate_additional_material_qty(self, material):
        ...
        return material.product_uom_qty * num_registrations if num_registrations else 0
    ...
    def create(self, vals_list):
        ...
        if isinstance(vals_list, dict):
            my_vals = vals_list
            sale_line = self.catch_sale_line_from_create_registration(vals_list)
            ...
            for val in vals_list:
                ...
                if sale_line and sale_line.event_ticket_id:
                    my_vals.append(val)
        return super().create(my_vals)

    def catch_sale_line_from_create_registration(self, vals):
        sale_line_obj = self.env["sale.order.line"]
        if "sale_order_line_id" in vals and vals.get("sale_order_line_id", False):
