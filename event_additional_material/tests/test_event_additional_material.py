...
class TestEventAdditionalMaterial(common.SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.event.registration_ids.write({"sale_order_id": cls.sale.id})
        cls.event.additional_material_ids = [
            (
                0,
                0,
                {"product_id": cls.product.id, "product_uom_qty": 2, "price_unit": 5},
            )
        ]
    ...
    def test_event_registration_action(self):
        ...
        self.assertEqual(self.sale.order_line[0].product_uom_qty, number * 2)
        self.assertEqual(self.sale.order_line[0].price_unit, 5)
        self.product.lst_price = 88
        self.event.additional_material_ids[0].onchange_product_id()
        self.assertEqual(self.event.additional_material_ids[0].price_unit, 88)
        ...
        wiz = self.wiz_obj.with_context(
            ...
            active_ids=self.event.registration_ids[0].ids,
        ).create({})
        wiz.action_put_material_from_registration()
        self.assertEqual(len(self.sale.order_line), 1)
        self.assertEqual(self.sale.order_line[0].product_uom_qty, number * 2)
        self.assertEqual(self.sale.order_line[0].price_unit, 88)
