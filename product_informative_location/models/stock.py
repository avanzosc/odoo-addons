...
class StockLocation(models.Model):
    ...
    has_zone = fields.Boolean(
        string='Has zone', default=False)
...
class StockMove(models.Model):
    ...
    def _catch_informative_location(self):
        ...
        my_description = ''
        code = (
            self.picking_id.picking_type_id.code if self.picking_id else
            self.picking_type_id.code)
        location_id = (
            self.picking_id.location_id if self.picking_id else
            self.location_id)
        location_dest_id = (
            self.picking_id.location_dest_id if self.picking_id else
            self.location_dest_id)
        if code in ('outgoing', 'internal'):
            lines += self.product_id.mapped(
                'product_location_ids').filtered(
                    lambda x: x.location_id == location_id)
            if lines:
                lines = lines.sorted('sequence')
                for line in lines:
                    my_text = (line.name if not my_text else
                               u'{}\n{}'.format(my_text, line.name))
                    my_description = (
                        line.description if not my_description else
                        u'{}\n{}'.format(my_description, line.description))
        if code in ('incoming', 'internal'):
            lines2 = self.product_id.mapped(
                'product_location_ids').filtered(
    ...
    def create(self, values):
        ...
        if not move.informative_location:
            ...
            if mytext or mydesc:
                move.write({'informative_location': mytext,
    ...
    def write(self, vals):
        ...
        if ('informative_location' not in self.env.context and
            'informative_location' not in vals and
            ('product_id' in vals or 'picking_type_id' in vals or
             'location_id' in vals or 'location_dest_id' in vals)):
            for move in self:
                mytext, mydesc = move._catch_informative_location()
                if (mytext != move.informative_location or
                        mydesc != move.informative_location_description):
...
class StockInventoryLine(models.Model):
    ...
    informative_location = fields.Text(
        string='Informative locations',
        compute='_compute_informative_locations')
        ...
        for inventory_line in self:
            if inventory_line.product_id and inventory_line.location_id:
                ...
                lines = inventory_line.product_id.mapped(
                    'product_location_ids').filtered(
                        lambda x: x.location_id == inventory_line.location_id)
                if lines:
                    lines = lines.sorted('sequence')
                    for line in lines:
                        if not my_text:
