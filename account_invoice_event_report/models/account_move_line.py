...
class AccountMoveLline(models.Model):
            ...
            if my_registrations:
                ...
                for reg in my_registrations:
                    students_name = (
                        ...
                        else "{}, {}".format(students_name, reg.student_id.name)
                    )
    ...
    @api.depends(
        "contract_line_id", "contract_line_id.sale_order_line_id", "sale_line_ids"
    )
    def _compute_sale_order_line_id(self):
        for line in self.filtered(
            lambda x: x.contract_line_id and x.contract_line_id.sale_order_line_id
        ):
            sale_line = line.contract_line_id.sale_order_line_id
            line.sale_order_line_id = sale_line.id
        for line in self.filtered(lambda x: not x.contract_line_id and x.sale_line_ids):
            if len(line.sale_line_ids) == 1:
                line.sale_order_line_id = line.sale_line_ids[0].id

    @api.depends(
        "sale_order_line_id",
        "sale_order_line_id.event_id",
        "sale_line_ids",
        "sale_line_ids.event_id",
    )
    def _compute_event_id(self):
        for line in self.filtered(
            lambda x: x.contract_line_id
            and x.sale_order_line_id
            and x.sale_order_line_id.event_id
        ):
            line.event_id = line.sale_order_line_id.event_id.id
        for line in self.filtered(lambda x: not x.contract_line_id and x.sale_line_ids):
            if len(line.sale_line_ids) == 1 and line.sale_line_ids[0].event_id:
