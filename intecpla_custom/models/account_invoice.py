...
class AccountInvoice(models.Model):
    ...
    def action_invoice_open(self):
        invoices = self.filtered(
            lambda x: x.type == 'in_invoice' and x.reference)
        if invoices:
            for invoice in invoices:
                cond = [('partner_id', '=', invoice.partner_id.id),
                ...
                if invs:
                    for inv in invs:
                        if (inv.date_invoice.year ==
                                fields.Date.context_today(self).year):
                            raise UserError(
                                _("The invoice already exists: %s, for the "
                                  "supplier: %s, with supplier reference: %s, "
                                  "and date %s.") % (
                                      inv.number, inv.partner_id.name,
                                      inv.reference, inv.date_invoice))
        ...
        for invoice in self:
            if invoice.type in ('in_invoice', 'in_refund'):
                result = super(AccountInvoice, self.with_context(
                    no_update_date=True)).action_invoice_draft()
            else:
    ...
    def write(self, vals):
        if 'no_catch_notes' in self.env.context:
            vals['comment'] = ' '
        if ('no_update_date' in self.env.context and 'date' in vals and not
                vals.get('date')):
            vals.pop('date')
...
class AccountInvoiceLine(models.Model):
    ...
    sale_order_section = fields.Char(
        string='Sale order section')
    repair_order_section = fields.Text(
        string='Repair order section')
    ...
    def create(self, values):
        ...
        if ('create_sale_order_section' not in self.env.context and
            'create_mrp_order_section' not in self.env.context and
            'origin' in values and values.get('origin') and
                'sale_line_ids' not in values):
            ...
            if repair:
                ...
                cond = [('repair_order_section', '=', repair_order_section),
                        ('origin', '=', my_origin),
        ...
        if ('create_sale_order_section' not in self.env.context and
            'create_mrp_order_section' not in self.env.context and
            'origin' in values and values.get('origin') and
                'sale_line_ids' in values):
            cond = [('name', '=', values.get('origin'))]
            sale = self.env['sale.order'].search(cond, limit=1)
            if sale:
                sale_date = self._convert_to_local_date(
                    sale.date_order, sale.user_id)
                sale_order_section = 'Order: {}, date: {}\n'.format(
                    sale.name, sale_date)
                values['sale_order_section'] = sale_order_section
                my_origin = values.get('origin')
                cond = [('sale_order_section', '=', sale_order_section),
                        ('origin', '=', my_origin),
                        ('invoice_id', '=', values.get('invoice_id'))]
                my_line = line_obj.search(cond)
                if not my_line:
    ...
    def create_line_with_sale_order_info(self, sale, sale_order_section,
                                         my_origin, values):
        sale_date = self._convert_to_local_date(sale.date_order, sale.user_id)
        my_name = _('Order: {}, date: {}').format(
            sale.name, sale_date)
        cond = [('invoice_id', '=', values.get('invoice_id'))]
        my_line = self.search(cond)
        if not my_line:
    ...
    def create_line_with_repair_order_info(self, repair, repair_order_section,
                                           my_origin, values):
        date_repair = self._convert_to_local_date(
            repair.date_repair, repair.user_id)
    ...
    def _ir_cron_fix_sale_order_section(self):
        cond = [('sale_order_section', 'ilike', '%Order: %date: %'),
        ...
        for line in lines:
            ...
            if sale:
                ...
                if line.sale_order_section != sale_order_section:
                    ...
                    cond = [('invoice_id', '=', line.invoice_id.id),
        ...
        cond = [('repair_order_section', 'ilike', '%Repair: %date: %'),
                ('invoice_id.origin', '!=', False)]
        lines = self.search(cond)
        modificados1 = 0
        modificados2 = 0
        for line in lines:
            ...
            if repair:
                date_repair = self._convert_to_local_date(
                    repair.date_repair, repair.user_id)
                repair_order_section = _(
                    'Repair: {}, date: {}').format(repair.name, date_repair)
    ...
    def _convert_to_local_date(self, mydate, user):
        ...
        return datetime.strptime(
            str(mydate), "%Y-%m-%d %H:%M:%S").strftime('%d-%m-%Y')
