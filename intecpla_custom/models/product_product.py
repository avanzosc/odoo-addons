...
class ProductProduct(models.Model):
    ...
    min_threshold_percentage_between_costs_id = fields.Many2one(
        ...
        comodel_name="threshold.percentage.between.costs", copy=False)
    ...
    product_color = fields.Selection(
        ...
        selection=[('none', _('Without color')),
                   ('green', _('Green')),
                   ('red', _('Red')),
                   ('orange', _('Orange'))],
        compute='_compute_product_color', store=True, related=False)
    notes_for_cost_sale_price = fields.Text(
        string="Notes for calculating Target Cost and Sale Price",
        copy=False, translate=False)

    @api.multi
    @api.depends('min_threshold_percentage_between_costs_id',
                 ...
                 'percentage_between_costs', 'manual_pvp')
    ...
    def name_get(self):
        ...
        def _name_get(d):
            name = d.get('name', '')
            code = (self._context.get('display_default_code', True) and
                    d.get('default_code', False) or False)
            if code:
                name = '[%s] %s' % (code, name)
            return (d['id'], name)
        ...
        for product in self.sudo():
            ...
            variant = product.attribute_value_ids._variant_name(
                variable_attributes)
            name = (variant and "%s (%s)" % (product.name, variant) or
                    product.name)
            mydict = {
                      'id': product.id,
                      'name': name,
        ...
        for product in products:
            product.write(
                {"min_threshold_percentage_between_costs_id":
                 min_threshold_percentage_between_costs.id,
                 "max_threshold_percentage_between_costs_id":
                 max_threshold_percentage_between_costs.id})
        return products
    ...
    def _ir_cron_english_translation_in_product(self):
        ...
        for translation in translations:
            if translation.source and translation.source != '*****':
                translation.write({'value': translation.source,
                                   'state': 'translated'})
        ...
        for arg in args:
            if isinstance(arg, (list, tuple)):
                if arg[0] == 'default_code':
                    ...
                    args = (
                        args[:index] +
                        ['|', ('description', arg[1], arg[2])] + args[index:])
                    break
