...
class StockMoveLine(models.Model):
    ...
    sale_order_id = fields.Many2one(
        string="Sale order", compute="_compute_sale_order_id",
        comodel_name="sale.order", copy=False, store=True
        )
    result_package_name = fields.Char(
        string="Package Referece", related="result_package_id.name",
        store=True, copy=False
        )
    parcel_number = fields.Char(
        string="Parcel number", copy=False)

    @api.depends("move_id", "move_id.sale_line_id",
    ...
    def get_info_to_print_picking_batch(self, lines, invoice_line):
        for line in self:
            qty = (line.qty_done if line.picking_id.state == "done" else
            ...
            total_weight_kg = round(
                container * package_qty * product_weight, 4)
            total_volume_m3 = round(volume_m3 * package_qty, 4)
            data = {"result_package": line.result_package_name,
            ...
            if invoice_line:
                data["invoice_line_number"] = (
                    invoice_line.sequence_related_invoice)
            lines.append(data)
        return lines

    def get_total_to_print_picking_batch(self, total_net_volume,
                                         total_gross_volume, total_net_weight,
                                         total_gross_weight):
        for line in self:
            volume_m3 = 0
            container = line.container
        ...
        return (total_net_volume, total_gross_volume, total_net_weight,
                total_gross_weight)
