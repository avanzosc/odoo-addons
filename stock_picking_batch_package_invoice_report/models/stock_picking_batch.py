...
class StockPickingBatch(models.Model):
    ...
    num_boxes = fields.Integer(
        string="Num. Boxes", compute="_compute_num_boxes",
        )
    stock_picking_batch_custom_package_ids = fields.One2many(
        string="Customized box name", inverse_name="picking_batch_id",
        comodel_name="stock.picking.batch.custom.package", copy=False
        )
    stock_picking_batch_total_box_ids = fields.One2many(
        string="Total boxes customization", inverse_name="picking_batch_id",
        comodel_name="stock.picking.batch.total.box", copy=False
        )
    total_gross_volume_m3 = fields.Float(
        string="Total Gross Volume (M3)", default=0.0, copy=False,
        digits=dp.get_precision('Product Price')
        )
    total_gross_weight_kg = fields.Float(
        string="Total Weight Volume (KG)", default=0.0, copy=False,
        digits=dp.get_precision('Product Price')
        )
    ...
    def _compute_num_boxes(self):
        for batch in self:
            ...
            if not batch.stock_picking_batch_total_box_ids:
                ...
                num_boxes = sum(batch.stock_picking_batch_total_box_ids.mapped(
                    "boxes_number"))
        ...
        for invline in invoice_lines:
            for move_line in self.move_line_ids:
                for invoice_line in move_line.move_id.invoice_line_ids:
                    if (invline == invoice_line and
                        invoice_line.move_id == invoice and
        ...
        for invoice in self.invoice_ids:
            ...
            for sale in sale_orders:
                ...
                for invoice_line in invoice_lines:
                    ...
                    if mls:
                        move_lines = mls.sorted(key=lambda x: (
                            x.result_package_name))
                        tnv, tgv, tnw, tgw = (
                            move_lines.get_total_to_print_picking_batch(
                                tnv, tgv, tnw, tgw))
        return {"total_net_volume": tnv,
