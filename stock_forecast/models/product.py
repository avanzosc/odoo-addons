...
class ProductTemplate(models.Model):
    ...
    def action_view_product_stock_forecast_from_template(self):
        ...
        action = self.env.ref("stock_forecast.action_product_stock_forecast").read()[0]
...
class ProductProduct(models.Model):
    ...
    def action_view_product_stock_forecast_from_product(self):
        self.ensure_one()
        self.env["product.product.stock.forecast"]._calc_qty_per_day(products_lst=self)
        action = self.env.ref("stock_forecast.action_product_stock_forecast").read()[0]
...
class ProductProductStockForecast(models.Model):
    ...
    def _store_qty(self, warehouse_id, min_fec, product):
        ...
        res = product.with_context(warehouse=warehouse_id.id)._compute_quantities_dict(
            None, None, None, None, fec
        )
        literal1 = _("Available: {}, Virtual: {}").format(
            res[product.id]["qty_available"], res[product.id]["virtual_available"]
        )
