...
class TestStockForecast(common.TransactionCase):
    ...
    def setUp(self):
        ...
        cond = [("code", "=", "incoming")]
        self.in_picking_type = self.env["stock.picking.type"].search(cond, limit=1)
        ...
        cond = [("code", "=", "outgoing")]
        self.out_picking_type = self.env["stock.picking.type"].search(cond, limit=1)
        cond = [("supplier", "=", True)]
        self.supplier = partner_obj.search(cond, limit=1)
        cond = [("customer", "=", True)]
        self.customer = partner_obj.search(cond, limit=1)
        vals = {
            "name": "Product for TestProductProductStockForecast",
            "type": "product",
        }
        ...
        picking_vals = {
            ...
            "location_dest_id": self.in_picking_type.default_location_dest_id.id,
        }
        picking_line_vals = {
        ...
        date_expected = fields.Datetime.from_string(
            self.picking.move_lines[0].date_expected
        ...
        self.out_picking.move_lines[0].write(
            {
                ...
                "date_expected_without_hour": self.picking2.move_lines[
                    0
    ...
    def test_stock_forecast_from_template(self):
        ...
        forecast = forecasts.filtered(
            lambda c: c.qty_available != 0
            or c.incoming_qty != 0
            or c.outgoing_qty != 0
            or c.virtual_available != 0
        )
