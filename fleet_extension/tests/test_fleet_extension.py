...
class TestFleetExtension(common.SavepointCase):
    ...
    def test_fleet_extension(self):
        ...
        self.assertEqual(self.serial_number1.product_id.id, self.vehicle1.product_id.id)
        self.vehicle1.product_id = (
            self.env["product.product"]
            .search([("id", "!=", self.vehicle1.product_id.id)], limit=1)
            .id
        )
        self.assertEqual(self.serial_number1.product_id.id, self.vehicle1.product_id.id)
        self.serial_number1.product_id = (
            self.env["product.product"]
            .search([("id", "!=", self.serial_number1.product_id.id)], limit=1)
            .id
        )
        self.assertEqual(self.serial_number1.product_id.id, self.vehicle1.product_id.id)
        self.assertEqual(self.vehicle2.serial_number_id.id, self.serial_number2.id)
