$(document).ready(function () {
  ...
  if (params.get("state") !== null) {
    $("#product_state_dropdown > option").each(function () {
      if (
        $(this)
          .val()
          .includes("state=" + params.get("state"))
      ) {
  ...
  if (params.get("member") !== null) {
    $("#product_member_dropdown > option").each(function () {
      if (
        $(this)
          .val()
          .includes("member=" + params.get("member"))
      ) {
  ...
  $("#products_grid > table > tbody > tr > td:nth-child(7) > form").submit(function (
    event
  ) {
  ...
  $(
    ".product-grid-card > div > div > div:nth-child(4) > div:nth-child(2) > form"
  ).submit(function (event) {
    var product_id = $(this)
      .closest(".card-body")
      .find("#product-grid-id")
      .text()
      .trim();
  ...
  $(".oe_website_sale > div.products_pager > div.dropdown_sorty_by > div > a").click(
    function () {
      var query = "?";
