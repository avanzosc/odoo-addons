...
class WebsiteSupplierZoneFilter(WebsiteSale):
    ...
    def shop(
        ...
        **post
        ...
        domain = self._get_search_domain(
            search, category, res.qcontext["attrib_values"]
        )
        url = "/shop"
        if category:
        ...
        product_template_ids = self.filterByMember(member, product_template_ids)
        product_count = len(product_template_ids)
        # Forced 4 products per page
        pager = request.website.pager(
            url=url, total=product_count, page=page, step=ppg, scope=7, url_args=post
        )
        pager_rec = self.recalculatePager(pager, zone, state, city, member, layout)
        product_ids = []
        for product in product_template_ids:
        ...
        product_ids = Product.search(
            domain,
            limit=ppg,
            offset=pager_rec["offset"],
            order=self._get_search_order(post),
        )
        res.qcontext.update(
            {"search_count": product_count, "pager": pager_rec, "products": product_ids}
        )
        res.qcontext.update(
            {
                "zone_ids": zone_ids,
    ...
    def recalculatePager(self, pager, zone, state, city, member, layout):
        ...
        pager["page_start"]["url"] = pager["page_start"]["url"] + synbol + page_filter
        pager["page_previous"]["url"] = (
            pager["page_previous"]["url"] + synbol + page_filter
        )
        pager["page_next"]["url"] = pager["page_next"]["url"] + synbol + page_filter
        pager["page_end"]["url"] = pager["page_end"]["url"] + synbol + page_filter
    ...
    def recalculatePagerLayout(self, pager, layout):
        ...
        if "layout=" not in pager["page"]["url"]:
            pager["page"]["url"] = pager["page"]["url"] + synbol + "layout=" + layout
            pager["page_start"]["url"] = (
                pager["page_start"]["url"] + synbol + "layout=" + layout
            )
            pager["page_previous"]["url"] = (
                pager["page_previous"]["url"] + synbol + "layout=" + layout
            )
            pager["page_next"]["url"] = (
                pager["page_next"]["url"] + synbol + "layout=" + layout
            )
            pager["page_end"]["url"] = (
                pager["page_end"]["url"] + synbol + "layout=" + layout
            )
            for page in pager["pages"]:
    ...
    def checkDeliveryCapacity(self, product_template_ids):
        ...
        if not (request.env.user._is_superuser() or request.env.user._is_admin()):
            for product in list(product_template_ids):
                search = False
                for supplier in product.seller_ids:
                    if (
                        supplier.name.sends_to == "all"
                        or (
                            supplier.name.sends_to == "city"
                            and supplier.supplier_city == partner.city
                        )
                        or (
                            supplier.name.sends_to == "zone"
                            and supplier.supplier_zone == partner.partner_zone_id
                        )
                        or (
                            supplier.name.sends_to == "state"
                            and supplier.supplier_state == partner.state_id
                        )
                    ):
                        search = True
                        break
