        ...
        for batch in self:
            ...
            if same:
                raise ValidationError(
                        _("Mother/Breeding already exists."))

    @api.depends("picking_ids", "picking_ids.state", "move_line_ids",
                 "move_line_ids.qty_done", "move_line_ids.product_id",
                 "move_line_ids.product_id.is_hen")
    def _compute_hen_unit(self):
        for batch in self:
            batch.hen_unit = 0
            if batch.move_line_ids and batch.batch_type == "mother":
                in_qty = sum(batch.move_line_ids.filtered(
                    lambda c: c.state == "done" and c.product_id.is_hen and (
                        c.location_dest_id == batch.location_id)
                    ).mapped("qty_done"))
                out_qty = sum(batch.move_line_ids.filtered(
                    lambda c: c.state == "done" and c.product_id.is_hen is (
                        True) and c.location_id == batch.location_id
                    ).mapped("qty_done"))
        ...
        for batch in self:
            ...
            if batch.start_date:
                ...
                if end_date < start_date:
                    start_date = datetime(
                        batch.start_date.year - 1, 1, 1, 0, 0).date()
                    start_date = batch.calculate_weeks_start(start_date)
                    end_date = datetime(
                        batch.start_date.year, 1, 1, 0, 0).date()
    ...
    def _compute_start_laying_weeks(self):
        for batch in self:
            ...
            if batch.start_laying_date:
                ...
                if end_date < start_date:
                    start_date = datetime(
                        batch.start_laying_date.year - 1, 1, 1, 0, 0).date()
                    start_date = batch.calculate_weeks_start(start_date)
                    end_date = datetime(
                        batch.start_laying_date.year, 1, 1, 0, 0).date()
                week = batch.weeks_between(start_date, end_date)
    ...
    def _compute_start_birth_weeks(self):
        for batch in self:
            ...
            if batch.start_birth_date:
                ...
                if end_date < start_date:
                    start_date = datetime(
                        batch.start_birth_date.year - 1, 1, 1, 0, 0).date()
                    start_date = batch.calculate_weeks_start(start_date)
                    end_date = datetime(
                        batch.start_birth_date.year, 1, 1, 0, 0).date()
                week = batch.weeks_between(start_date, end_date)
    ...
    def _compute_change_house_weeks(self):
        for batch in self:
            ...
            if batch.change_house_date:
                ...
                if end_date < start_date:
                    start_date = datetime(
                        batch.change_house_date.year - 1, 1, 1, 0, 0).date()
                    start_date = batch.calculate_weeks_start(start_date)
                    end_date = datetime(
                        batch.change_house_date.year, 1, 1, 0, 0).date()
                week = batch.weeks_between(start_date, end_date)
    ...
    def _compute_end_rearing_weeks(self):
        for batch in self:
            ...
            if batch.end_rearing_date:
                ...
                if end_date < start_date:
                    start_date = datetime(
                        batch.end_rearing_date.year - 1, 1, 1, 0, 0).date()
                    start_date = batch.calculate_weeks_start(start_date)
                    end_date = datetime(
                        batch.end_rearing_date.year, 1, 1, 0, 0).date()
                week = batch.weeks_between(start_date, end_date)
    ...
    def _compute_end_laying_weeks(self):
        for batch in self:
            ...
            if batch.end_laying_date:
                ...
                if end_date < start_date:
                    start_date = datetime(
                        batch.end_laying_date.year - 1, 1, 1, 0, 0).date()
                    start_date = batch.calculate_weeks_start(start_date)
                    end_date = datetime(
                        batch.end_laying_date.year, 1, 1, 0, 0).date()
                week = batch.weeks_between(start_date, end_date)
    ...
    def _compute_end_birth_weeks(self):
        for batch in self:
            ...
            if batch.end_birth_date:
                ...
                if end_date < start_date:
                    start_date = datetime(
                        batch.end_birth_date.year - 1, 1, 1, 0, 0).date()
                    start_date = batch.calculate_weeks_start(start_date)
                    end_date = datetime(
                        batch.end_birth_date.year, 1, 1, 0, 0).date()
                week = batch.weeks_between(start_date, end_date)
    ...
    def _compute_closing_weeks(self):
        for batch in self:
            ...
            if batch.closing_date:
                ...
                if end_date < start_date:
                    start_date = datetime(
                        batch.closing_date.year - 1, 1, 1, 0, 0).date()
                    start_date = batch.calculate_weeks_start(start_date)
                    end_date = datetime(
                        batch.closing_date.year, 1, 1, 0, 0).date()
                week = batch.weeks_between(start_date, end_date)
