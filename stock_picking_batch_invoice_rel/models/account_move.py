...
class AccountMove(models.Model):
    ...
    stock_picking_batch_ids = fields.Many2many(
        string="Batch Transfers", comodel_name="stock.picking.batch",
        relation="rel_picking_batch_invoice", column1="invoice_id",
        column2="picking_batch_id"
        )
    stock_picking_batch_count = fields.Integer(
        string="Batch Transfers Counter",
        compute="_compute_stock_picking_batch_count"
        )

    def _compute_stock_picking_batch_count(self):
        for invoice in self:
            invoice.stock_picking_batch_count = len(
                invoice.stock_picking_batch_ids)
