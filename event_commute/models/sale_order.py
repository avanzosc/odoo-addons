        ...
        for sale in sales:
            if sale.tasks_count == 1 and sale.state == "sale":
                lines_with_event = sale.order_line.filtered(
                    ...
                    and x.product_id.service_policy == "delivered_timesheet"
                    and x.product_id.service_tracking == "task_in_project"
                    and x.product_id.event_ok
                )
                lines = sale.order_line.filtered(
                    lambda x: x.project_id
                    and x.task_id
                    and not x.event_ticket_id
                    and x.product_id.type == "service"
                    and x.product_id.service_policy == "delivered_timesheet"
                    and x.product_id.service_tracking == "task_in_project"
                    and not x.product_id.event_ok
                )
                if (
                    lines
                    and len(lines_with_event) == 1
                    and not lines_with_event[0].event_id.shared_price_event
                ):
                    for line in lines:
                        vals = {
                            "order_id": sale.id,
                        ...
                        new_line.write(vals)
                        cond = [
                            ("event_id", "=", lines_with_event[0].event_id.id),
                        ...
                        self.env.cr.execute(
                            "DELETE FROM sale_order_line " "WHERE id = %s", [line.id]
                        )
                        self.env.cr.commit()
