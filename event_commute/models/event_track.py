...
class EventTrack(models.Model):
    ...
    def write(self, vals):
        ...
        for track in self:
            ...
            if not user:
                raise UserError(
                    _("User not found for speaker: {}").format(track.partner_id.name)
                )
            cond = [("user_id", "=", user.id)]
            employee = self.env["hr.employee"].search(cond, limit=1)
            if not employee:
                raise UserError(_("Employee not found for user: {}").format(user.name))
        ...
        if self.event_id.displacement_product_ids:
            for line in self.event_id.displacement_product_ids:
                ...
                if not line.project_id and self.event_id.project_id:
                    cond.append(("project_id", "=", self.event_id.project_id.id))
                    if self.event_id.task_id:
                        cond.append(("task_id", "=", self.event_id.task_id.id))
                lines = analytic_line_obj.search(cond)
                if lines:
                    lines.unlink()
                vals = self._catch_values_for_create_analytic_line(self.partner_id)
                name = vals.get("name")
                ...
                analytic_line = self.create_analytic_line_from_track(line, vals)
    ...
    def create_analytic_line_from_track(self, line, vals):
        return (
            ...
            .with_context(
                default_sale_order_id=line.sale_order_id, default_track_id=self
            )
            .create(vals)
        )
