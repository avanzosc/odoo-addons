...
class CouponProgram(models.Model):
    ...
    apply_on_total = fields.Boolean(
        "Apply on total",
        help="Apply coupon on the total amount of the SO,"
        " in the SO line sequence order.",
    )

    coupon_program_groups = fields.Many2many(
        "coupon.program.group",
        "coupon_program_group_rel",
        "program_id",
        "group_id",
        string="Coupon program Groups",
    )
    ...
    def _check_group_promo_code(self, order, coupon_code, group):
        ...
        if order._get_applicable_programs() in group.coupon_programs:
            message = {"error": _("Promo group already applied.")}
        ...
        #     message = {'error': _('already_applied')}
            ...
            self.maximum_use_number != 0 and self.order_count >= self.maximum_use_number
        ):
