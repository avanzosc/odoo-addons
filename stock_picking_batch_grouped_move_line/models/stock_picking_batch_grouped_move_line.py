...
class StockPickingWaveGroupedMoveLine(models.Model):
    ...
    packages = fields.Char(
        string="Source package", copy=False
    )
    product_id = fields.Many2one(
        string="Product", comodel_name="product.product", copy=False
    )
    product_uom_id = fields.Many2one(
        string="UoM", comodel_name="uom.uom", store=True, copy=False,
        related="product_id.uom_id"
    )
    lots = fields.Char(
        string="Lots/Serial numbers", copy=False
    )
    owners = fields.Char(
        string="Owners", copy=False
    )
    qty_done = fields.Float(
        string="Done", digits="Product Unit of Measure", default=0.0,
        copy=False
    )
    ...
    result_packages = fields.Char(
        string="Result packages", copy=False
    )
