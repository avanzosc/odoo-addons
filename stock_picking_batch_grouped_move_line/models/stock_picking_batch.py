...
class StockPickingBatch(models.Model):
    ...
    grouped_move_line_ids = fields.Many2many(
        ...
        relation="rel_picking_batch_move_grouped", column1="batch_id",
        column2="grouped_move_line_id", copy=False,
    ...
    def _compute_stock_move_line_grouped(self):
        ...
        for batch in self:
            ...
            for ope in batch.move_line_ids.filtered(
                lambda x: x.product_id and x.location_id and
                    x.location_dest_id):
                key = "{}-{}-{} ".format(
                    ope.product_id.id, ope.location_id.id,
                    ope.location_dest_id.id)
                if key not in lines:
                    ...
                    result_packages = (
                        ope.result_package_id.name if
                        ope.result_package_id else "")
                    vals = {
                        "pickings": pickings,
                        "packages": packages,
                    ...
                    if ope.picking_id and ope.picking_id.name not in pickings:
                        pickings = (
                            u"{}, {}".format(pickings, ope.picking_id.name) if
                            pickings else ope.picking_id.name)
                    if ope.package_id and ope.package_id.name not in packages:
                        packages = (
                            u"{}, {}".format(packages, ope.package_id.name) if
                            packages else ope.package_id.name)
                    if ope.lot_id and ope.lot_id.name not in lots:
                        lots = (
                            u"{}, {}".format(lots, ope.lot_id.name) if
                            lots else ope.lot_id.name)
                    if ope.owner_id and ope.owner_id.name not in owners:
                        owners = (
                            u"{}, {}".format(owners, ope.owner_id.name) if
                            owners else ope.owner_id.name)
                    if (ope.result_package_id and
                            ope.result_package_id.name not in result_packages):
                        result_packages = (
                            u"{}, {}".format(
                                result_packages,
                                ope.result_package_id.name) if result_packages
