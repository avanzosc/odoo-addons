...
class ProductProduct(models.Model):
    ...
    name2 = fields.Char(
        string='Product name on sales', translate=True)
    ...
    def create(self, values):
        ...
        if 'name2' not in values or not values.get('name2', ''):
             values['name2'] = '*****'
        if ('description_sale_es' not in values or not
                values.get('description_sale_es', '')):
            if values.get('name2') == '*****':
        ...
        if ('description_sale_en' not in values or not
                values.get('description_sale_en', '')):
            if values.get('name2') == '*****':
        ...
        if ('description_sale_cat' not in values or not
                values.get('description_sale_cat', '')):
            if values.get('name2') == '*****':
    ...
    def write(self, values):
        ...
        if "description_sale_es" in values:
            super(ProductProduct, self.with_context(
                lang="es_ES", language_description=True)).write(
                    {'name2': values.get("description_sale_es")})
        if "description_sale_cat" in values:
            super(ProductProduct, self.with_context(
                lang="ca_ES", language_description=True)).write(
        ...
        if "description_sale_en" in values:
            for product in self:
                ...
                if translation:
                    ...
                    translation.sudo().with_context(
                        language_description=True).write(translation_vals)
        result = super(ProductProduct, self).write(values)
        if ("update_name2_data_from_template" not in self.env.context and
           ("name2" in values or "description_sale_es" in values or
            "description_sale_cat" in values or
                "description_sale_en" in values)):
            for product in self.filtered(
                    lambda x: x.product_tmpl_id.product_variant_count == 1):
                if "name2" in values:
                    product.product_tmpl_id.with_context(
                        update_name2_data_from_product=True).write(
                            {'name2': values.get("name2")})
                if "description_sale_es" in values:
                    product.product_tmpl_id.with_context(
                        update_name2_data_from_product=True).write(
                            {'description_sale_es': values.get(
                                "description_sale_es")})
                if "description_sale_cat" in values:
                    product.product_tmpl_id.with_context(
                        update_name2_data_from_product=True).write(
                            {'description_sale_cat': values.get(
                                "description_sale_cat")})
                if "description_sale_en" in values:
                    product.product_tmpl_id.with_context(
                        update_name2_data_from_product=True).write(
                            {'description_sale_en': values.get(
                                "description_sale_en")})
        return result
    ...
    def _ir_cron_name_products_by_language(self):
        ...
        for template in templates:
            template.with_context(
                ...
                from_cron=True).put_description_sale_lang()
            if template.product_variant_count == 1:
                ...
                if translations:
                    translations.unlink()
                cond = [('name', '=', 'product.template,name2'),
                       ('type', '=', 'model'),
                       ('res_id', '=', template.id)]
                translations = translation_obj.search(cond)
                if translations:
                    for translation in translations:
                        ...
                        if translation.lang == 'es_ES':
                            variant.with_context(
                                from_cron=True).description_sale_es = (
                                    template.description_sale_es)
                        if translation.lang == 'ca_ES':
                            variant.with_context(
                                from_cron=True).description_sale_cat = (
                                    template.description_sale_cat)
                        if translation.lang == 'en_US':
                            variant.with_context(
                                from_cron=True).description_sale_en = (
                                    template.description_sale_en)
                template.product_variant_ids[0].with_context(
                    from_cron=True).last_date_description_for_sale_by_language = (
                        fields.Date.context_today(self))
            template.with_context(
                from_cron=True).last_date_description_for_sale_by_language = (
                    fields.Date.context_today(self))
