...
class ProductTemplate(models.Model):
    ...
    def create_translations(self):
        ...
        for lang in langs:
            ...
            if not translation:
                vals.update({'name': 'product.template,name2',
                             'type': 'model',
                             'res_id': self.id,
                             'lang': lang.code,
                             'scr': self.name2})
    ...
    def write(self, values):
        ...
        if "description_sale_es" in values:
            super(ProductTemplate, self.with_context(
            ...
            for template in self.filtered(
                ...
                if "name2" in values:
                    template.product_variant_ids[0].with_context(
                        update_name2_data_from_template=True).write(
    ...
    def put_description_sale_lang(self):
        for template in self:
            ...
            if translation:
                if not translation.value or translation.value == '*****':
                    translation.with_context(
                        language_description=True).write(
                            {'src': '*****',
                             'value': '*****',
                             'state': 'to_translate'})
            ...
            if not description_sale_es:
                ...
                if translation:
                    translation.with_context(
                        language_description=True).copy(
                            {'src': '*****',
                             'lang': 'es_ES',
                             'value': '*****',
                             'state': 'to_translate'})
            ...
            if description_sale_es:
                template.with_context(
                    from_cron=True).description_sale_es = description_sale_es
            ...
            if translation:
                if not translation.value or translation.value == '*****':
                    ...
                    if (translation.value and
                            translation.src != description_sale_es):
                        translation.with_context(
                            language_description=True).write(
                                {'state': 'translated',
                                 'src': description_sale_es})
            ...
            if description_sale_cat:
                template.with_context(
                    from_cron=True).description_sale_cat = description_sale_cat
            ...
            if description_sale_en:
                template.with_context(
                    from_cron=True).description_sale_en = description_sale_en
