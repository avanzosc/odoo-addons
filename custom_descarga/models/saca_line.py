...
class SacaLine(models.Model):
    ...
    def _get_selection_color(self):
        return self.env["saca.line.stage"].fields_get(
            allfields=["color_name"])["color_name"]["selection"]
    ...
    color_name = fields.Selection(
        string="Color",
        selection="_get_selection_color",
    ...
    hard_chicken = fields.Float(
        string="Hard Chicken %", copy=False)
    ...
    img_dest = fields.Binary(
        string="Ticket Slaughterhouse", copy=False)
    staff_crew = fields.Integer(
        string="Staff Crew", copy=False)
        ...
        if self.stage_id == stage_matanza:
            ...
            for picking in self.sudo().sale_order_id.picking_ids:
                for move in picking.move_ids_without_package:
                    name = u"{}".format(self.sudo().breeding_id.name)
            ...
            type_normal = self.env["sale.order.type"].sudo().search([
                ("name", "ilike", "Normal"),
