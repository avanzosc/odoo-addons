...
class WizEventAttendanceReport(models.TransientModel):
    ...
    def filter_lines(self, lines):
        ...
        if self.level_id:
            lines = lines.filtered(lambda x: x.level_id.id == self.level_id.id)
        if self.lang_id:
            lines = lines.filtered(lambda x: x.lang_id.id == self.lang_id.id)
        return lines
