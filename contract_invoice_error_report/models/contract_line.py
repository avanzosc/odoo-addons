...
class ContractLine(models.Model):
    ...
    def _get_period_to_invoice_error(self, last_date_invoiced,
                                     recurring_next_date,
        ...
        if not recurring_next_date:
            ...
            error = (
                _("Contract without recurring next date"))
        ...
        if not first_date_invoiced:
            error = (
                _("Contract without first date invoiced. Last date "
                  "invoice: {}, Date start: {}. How result without first "
                  "date invoiced: {}.").format(
                      my_last_date_invoiced, self.date_start,
                      first_date_invoiced))
            self.contract_id.sudo().write(
                {"with_invoice_generation_error": True,
                 "invoice_generation_error": error})
        ...
        if max_date_end and first_date_invoiced > max_date_end:
            error = (
                _("Next period date start > Max. Date end. Last date "
                  "invoice: {}, Date start: {}. How result Next period date "
                  "start: {}. Stop at date end: {}, Line date end: {}. How "
                  "result Max. Date end: {}.").format(
                      my_last_date_invoiced, self.date_start,
                      first_date_invoiced, stop_at_date_end, self.date_end,
                      max_date_end))
            self.contract_id.sudo().write(
                {"with_invoice_generation_error": True,
                 "invoice_generation_error": error})
