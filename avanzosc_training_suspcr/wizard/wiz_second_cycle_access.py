...
class wiz_second_cycle_access(osv.osv_memory):
    _name = 'wiz.second.cycle.access'
    _description = 'wiz second cycle access'
    ...
    def _find_call(self, cr, uid, seance, record_id=False):
        ...
        no_school=False
        if record_id:
             record = training_record_obj.browse(cr, uid, record_id)
    ...
    def default_get(self, cr, uid, fields_list, context=None):
        ...
        training_record_second_cycle_access_obj = self.pool.get('training.record.second.cycle.access')
        ...
        if 'state' in context:
            ...
            session = sale.session_id
        #Busacamos el Expediente y cogemos el primero    
        ...
        #Buscamos las asignaturas de acceso a segundo ciclo
        second_cycle_ids = training_record_second_cycle_access_obj.search(cr, uid, [('record_id','=',record_ids[0])])
        if second_cycle_ids:
            for i in training_record_second_cycle_access_obj.browse(cr,uid,second_cycle_ids):
                training_course_items.append(i.course_id.id)
        
        for seance in session.seance_ids:
            if seance.course_id.id in training_course_items:
...
wiz_second_cycle_access()

class wiz_training_subject_second_cycle_master(osv.osv_memory):
    _name = 'wiz.training.subject.second.cycle.master'
...
#                'convalidate': False,
        ...
        return {'value': res}
wiz_training_subject_second_cycle_master()

class wiz_training_fee_second_cycle_master(osv.osv_memory):
    _name = 'wiz.training.fee.second.cycle.master'
...
wiz_training_fee_second_cycle_master()

class wiz_training_recog_second_cycle_master(osv.osv_memory):
    _name = 'wiz.training.recog.second.cycle.master'
...
wiz_training_recog_second_cycle_master()