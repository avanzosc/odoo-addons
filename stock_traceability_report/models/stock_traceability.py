...
class MrpStockReport(models.TransientModel):
    ...
    def _final_vals_to_lines(self, final_vals, level):
        ...
        for key, data in enumerate(final_vals):
            cols = lines[key].get('columns').append(data.get('partner_id', ''))
        return lines
    ...
    def _make_dict_move(self, level, parent_id, move_line, unfoldable=False):
        ...
        if move_line.move_id.picking_partner_id:
            res[0].update({
                'partner_id': move_line.move_id.picking_partner_id.display_name,
            })
        return res
