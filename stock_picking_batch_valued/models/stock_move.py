...
class StockMove(models.Model):
    ...
    sale_price_unit = fields.Float(
        string="Sale Unit Price", digits="Product Price",
        related="sale_line_id.price_unit", store=True, copy=False)
    sale_price_subtotal = fields.Float(
        string="Sale Subtotal", digits="Product Price", store=True, copy=False,
    ...
    def _compute_sale_price_subtotal(self):
        for move in self:
            ...
            if move.sale_price_unit > 0:
                if move.state == "done":
                    sale_price_subtotal = (
                        move.quantity_done * move.sale_price_unit)
