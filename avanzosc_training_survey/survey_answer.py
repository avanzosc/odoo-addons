...
class survey_question_wiz(osv.osv_memory):
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        ...
        fields_view_getans_obj = self.pool.get('survey.answer')
        ...
        if view_type in ['form']:
            ...
            ###################################################
            #              CODIGO AvanzOSC  (INICIO)          #
            ###################################################
            
            answer_list_id = False
            if 'answer_list_id' in context:
                answer_list_id = context['answer_list_id']
            else:
                answer_list_obj = self.pool.get('survey.answer.list')
                
                address = False
                partner = False
                
                if sur_name_rec.address_id:
                    address = sur_name_rec.address_id.id
                if sur_name_rec.partner_id:
                    partner = sur_name_rec.partner_id.id
                ...
                answer_list_id = answer_list_obj.create(cr, uid, val)
            context.update({'answer_list_id': answer_list_id})
            ###################################################
            #              CODIGO AvanzOSC  (FIN)             #
            ###################################################
                ...
                if flag:
                    ...
                    if context.has_key('response_id') and context.get('response_id', False) \
                        ...
                        if context.get('response_no',0) + 1 < len(context.get('response_id',0)):
                            etree.SubElement(xml_group, 'button', {'colspan':"1",'icon': "gtk-go-forward", 'name':"action_forward_next",'string': tools.ustr("Next Answer") ,'type':"object",'context' : str(context)})
                    ...
                    for que in que_ids:
                        ...
                        if context.has_key('active') and context.get('active',False) and \
                            ...
                            etree.SubElement(xml_group1, 'button', {'string':'','icon': "gtk-edit", 'type' :'object', 'name':"action_edit_question", 'context' : str(context)})
                            etree.SubElement(xml_group1, 'button', {'string':'','icon': "gtk-delete", 'type' :'object','name':"action_delete_question", 'context' : str(context)})
                        else:
                            xml_group = etree.SubElement(xml_form, 'group', {'col': '1', 'colspan': '4'})
                        ...
                        if que_rec.type == 'multiple_choice_only_one_ans':
                            ...
                            for ans in ans_ids:
                                ...
                                if type == "char" :
                                    ...
                                    fields[tools.ustr(que.id) + "_" + resulttools.ustr(ans.id) + "_multi"] = {'type': str(type), 'string':ans.answer}
                    ...
                    if context.has_key('active') and context.get('active',False) and int(page_number) + 1 == total_pages and context.has_key('response_id') and context.has_key('response_no') and  context.get('response_no',0) + 1 == len(context.get('response_id',0)):
                        etree.SubElement(xml_group, 'button', {'icon': "gtk-go-forward", 'special' : 'cancel','string': tools.ustr("Done") ,'context' : str(context)})
                        ...
                        etree.SubElement(xml_group, 'button', {'icon': "gtk-go-forward", 'name':"action_forward_next",'string': tools.ustr("Next Answer") ,'type':"object",'context' : str(context)})
                    elif context.has_key('active') and context.get('active',False) and int(page_number) + 1 == total_pages:
                        etree.SubElement(xml_group, 'button', {'icon': "gtk-go-forward", 'special': "cancel", 'string' : 'Done', 'context' : str(context)})
                    else:
                        etree.SubElement(xml_group, 'button', {'icon': "gtk-go-forward", 'name':"action_next",'string': tools.ustr(but_string) ,'type':"object",'context' : str(context)})
                    ...
                    if context.has_key('active') and context.get('active',False) and context.has_key('edit'):
                        ...
                        etree.SubElement(xml_group3, 'button', {'string' :'Add Page','icon': "gtk-new", 'type' :'object','name':"action_new_page", 'context' : str(context)})
                        etree.SubElement(xml_group3, 'button', {'string' :'Edit Page','icon': "gtk-edit", 'type' :'object','name':"action_edit_page", 'context' : str(context)})
                        etree.SubElement(xml_group3, 'button', {'string' :'Delete Page','icon': "gtk-delete", 'type' :'object','name':"action_delete_page", 'context' : str(context)})
                        etree.SubElement(xml_group3, 'button', {'string' :'Add Question','icon': "gtk-new", 'type' :'object','name':"action_new_question", 'context' : str(context)})
                    ...
                    for key1, val1 in vals.items():
                        if val1 and key1.split('_')[1] == "table" and key1.split('_')[0] == que_id:
                            ...
                            if len(key1.split('_')) > 2:
                                ans_create_id = res_ans_obj.create(cr, uid, {'response_id':resp_id, 'answer_id':key1.split('_')[-1], 'column_id' : val1, 'list_id':answer_list_id})
                                selected_value.append(val1)
                                response_list.append(str(ans_create_id) + "_" + str(key1.split('_')[-1]))
                            else:
                                ans_create_id = res_ans_obj.create(cr, uid, {'response_id':resp_id, 'answer_id':val1, 'list_id':answer_list_id})
                            sur_name_read['store_ans'][resp_id].update({key1:val1})
                            select_count += 1
                            ...
                            if key1.split('_')[1] == "single" :
                                ...
                                ans_create_id = res_ans_obj.create(cr, uid, {'response_id':resp_id, 'answer_id':key1.split('_')[1], 'answer' : val1, 'list_id':answer_list_id})
                            ...
                            if type(val1) == type('') or type(val1) == type(u''):
                                ans_create_id = res_ans_obj.create(cr, uid, {'response_id':resp_id, 'answer_id':key1.split('_')[1], 'column_id' : key1.split('_')[2], 'value_choice' : val1, 'list_id':answer_list_id})
                                sur_name_read['store_ans'][resp_id].update({key1:val1})
                            else:
                                ans_create_id = res_ans_obj.create(cr, uid, {'response_id':resp_id, 'answer_id':key1.split('_')[1], 'column_id' : key1.split('_')[2], 'list_id':answer_list_id})
                    ...
                    if ans_id_len[0] == sur_name_read['store_ans'][update]['question_id']:
                        if val and key.split('_')[1] == "table":
                            ...
                            if len(key.split('_')) > 2:
                                ans_create_id = res_ans_obj.create(cr, uid, {'response_id':update, 'answer_id':key.split('_')[-1], 'column_id' : val, 'list_id':answer_list_id})
                                selected_value.append(val)
                                response_list.append(str(ans_create_id) + "_" + str(key.split('_')[-1]))
                            else:
                                ans_create_id = res_ans_obj.create(cr, uid, {'response_id':update, 'answer_id': val, 'list_id':answer_list_id})
                            resp_obj.write(cr, uid, update, {'state': 'done'})
                            sur_name_read['store_ans'][update].update({key:val})
                            select_count += 1
                            ...
                            if key.split('_')[1] == "single" :
                                ...
                                resp_obj.write(cr, uid, update, {'state': 'done'})
                                ans_create_id = res_ans_obj.create(cr, uid, {'response_id':update, 'answer_id':ans_id_len[1], 'answer' : val, 'list_id':answer_list_id})
                            sur_name_read['store_ans'][update].update({key:val})
                            select_count += 1
                            ...
                            if type(val) == type('') or type(val) == type(u''):
                                ans_create_id = res_ans_obj.create(cr, uid, {'response_id': update, 'answer_id':ans_id_len[1], 'column_id' : ans_id_len[2], 'value_choice' : val, 'list_id':answer_list_id})
                                sur_name_read['store_ans'][update].update({key:val})
                            else:
                                ans_create_id = res_ans_obj.create(cr, uid, {'response_id': update, 'answer_id':ans_id_len[1], 'column_id' : ans_id_len[2], 'list_id':answer_list_id})
