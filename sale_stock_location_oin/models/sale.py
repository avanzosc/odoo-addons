    ...
    def _check_product_location(self):
        for order in self:
            for line in order.order_line.filtered(
                lambda x: x.location_id and x.sale_type_id and x.product_id
            ):
                ...
                if lines_count > 1:
                    raise ValidationError(
                        _(
                            """You cannot add same product %s with
...
class StockRule(models.Model):
    ...
    def _get_stock_move_values(
        ...
        if "sale_line_id" in values and values.get("sale_line_id", False):
            sale_line_id = (
                self.env["sale.order.line"].sudo().browse(values["sale_line_id"])
            )
        ...
        if values.get("sale_line_id", False) and sale_line_id.sale_type_id:
            values["picking_type_id"] = sale_line_id.sale_type_id.picking_type_id.id
        return values
