...
class ResourceCalendar(models.Model):
    ...
    hour_gap = fields.Float(string="Weekly Hour Gap", compute="_compute_hour_gap")
    ...
    @api.depends(
        ...
        "attendance_ids.date_from",
        "attendance_ids.date_to",
    )
    def _compute_hour_gap(self):
        today = fields.Date.context_today(self)
        year, week_num, day_of_week = today.isocalendar()
        start_dt = datetime.strptime(
            "{}-W{}-1".format(year, week_num - 1), "%Y-W%W-%w"
        ).replace(tzinfo=utc)
...
class ResourceCalendarAttendance(models.Model):
    ...
    hour_gap = fields.Float(string="Hour Gap", compute="_compute_hour_gap", store=True)
    delay = fields.Integer(string="Allowed hours extend entry", default=0)
    delay_to = fields.Integer(string="Allowed hours extend departure", default=0)
    ...
    def _compute_delay_hour_from_to(self):
        for record in self:
            ...
            delay_to = record.delay_to if record.delay_to > 0 else record.delay_to * -1
