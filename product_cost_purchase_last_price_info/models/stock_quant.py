...
class StockQuant(models.Model):
    ...
    last_purchase_price_company_currency = fields.Float(
        ...
        groups="stock.group_stock_manager", store=True, copy=False,
    )
    purchase_value = fields.Monetary(
        compute="_compute_purchase_value", string="Last purchase price value",
        groups="stock.group_stock_manager", store=True, copy=False,
    )
    standard_price = fields.Float(
        string="Product Price",
        related="product_id.standard_price", store=True, copy=False,
        groups="stock.group_stock_manager"
    )
    value = fields.Monetary(
        string="Product cost price value"
    )
    value_to_pivot = fields.Monetary(
        string="Product cost price value", related="value", store=True,
        copy=False)

    @api.depends("location_id", "quantity",
                 "product_id",
    ...
    def _compute_purchase_value(self):
        for quant in self:
            ...
            quant.sudo().purchase_value = (
                quant.quantity *
                round(quant.product_id.last_purchase_price_company_currency, 2))
