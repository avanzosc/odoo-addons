...
def _merge_orders(self, cr, uid, data, context):
    ...
    for porder in [order for order in order_obj.browse(cr, uid, data['ids']) if order.state == 'draft']:
        ...
        for order_line in porder.order_line:
            line_key = make_key(order_line,('name', 'date_planned', 'taxes_id',
                                             'price_unit', 'notes', 'product_id',
                                             'move_dest_id', 'account_analytic_id',
                                             'size_x', 'size_y', 'size_z', 'shape',
                                             'density', 'diameter','production_id','weight'))
...
merge_orders("mrp_dimensions.order.merge")
