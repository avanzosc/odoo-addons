...
class sale_order(osv.osv):
    ...
    def action_ship_create(self, cr, uid, ids, *args):
        ...
        picking_id = False
        picking_id = super(sale_order, self).action_ship_create(cr, uid, ids, *args)
        ...
        for order in self.browse(cr, uid, ids):
            for order_line in order.order_line:
                ...
                if order_line.product_id.product_tmpl_id.type in ('product', 'consu'):
                    stock_move_id = self.pool.get('stock.move').search(cr, uid, [('sale_line_id', '=', order_line.id)])
                    if stock_move_id:
...
class purchase_order_line(osv.osv):
    ...
    _columns = {
        ...
        'diameter' : fields.float('Diameter',),
        'weight': fields.float('Weight',),
        'purchase_price': fields.selection((('weight', 'Weight'), ('units', 'Units')), 'Price in', required=True),
...
class mrp_bom(osv.osv):
    ...
    def onchange_product_id(self, cr, uid, ids, product_id, name, id, context={}):
        if product_id:
