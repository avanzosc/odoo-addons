...
class StockReturnPicking(models.TransientModel):
    ...
    def _create_returns(self):
        ...
        for line in self.picking_id.move_line_ids.filtered(
            lambda x: x.state == "done" and x.is_repair and x.lot_id and
                x.sale_line_id):
            for new_line in new_picking.move_line_ids.filtered(
                lambda z: z.product_id == line.product_id and not
                    z.lot_id):
                new_line.lot_id = line.lot_id.id
                new_line._onchange_serial_number()
                break
        if (self.picking_id.is_repair and self.picking_id.sale_order_id and
