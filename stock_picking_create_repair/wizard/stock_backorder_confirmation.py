...
class StockBackorderConfirmation(models.TransientModel):
    ...
    def process(self):
        ...
        pickings = self.pick_ids.filtered(
            ...
            x.is_repair and x.sale_order_id)
        if pickings:
            pickings.with_context(
                created_new_picking=True)._put_realized_moves_in_repairs()
        return result
        ...
        if pickings:
            for picking in pickings:
                raise ValidationError(
                    _("The out picking {} is for repairs, you must create a "
                      "partial order delivery").format(picking.name))
