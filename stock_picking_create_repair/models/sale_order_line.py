...
class SaleOrderLine(models.Model):
    ...
    repair_amount_untaxed = fields.Monetary(
        string='Repair untaxed amount', copy=False, store=True,
    ...
    repair_picking_move_line_ids = fields.One2many(
        string="Stock move lines from repair pickings", copy=False,
        comodel_name="stock.move.line", inverse_name="sale_line_id")
    service_repair_product_id = fields.Many2one(
        string="Service repair product", comodel_name="product.product",
        copy=False)

    @api.depends("repair_order_ids", "repair_order_ids.amount_untaxed",
                 "repair_order_ids.invoice_method", "repair_order_ids.state")
    def _compute_repair_amount_untaxed(self):
        for line in self:
            repair_amount_untaxed = 0
            if (line.is_service and line.is_repair and
                    line.product_to_repair_id):
                repairs = line.repair_order_ids.filtered(
                    lambda x: x.invoice_method != 'none' and
                    x.state != 'cancel')
                if repairs:
                    repair_amount_untaxed = sum(
                        repairs.mapped("amount_untaxed"))
    ...
    @api.depends("product_uom_qty", "qty_delivered_method", "qty_delivered",
                 "price_unit", "discount", "repair_order_ids",
    ...
    def _compute_qty_amount_pending_delivery(self):
        ...
        for line in self:
            if (line.is_service and line.is_repair and
                    line.product_to_repair_id and line.order_id.is_repair):
                ...
                line.amount_pending_delivery = (
                    line._get_amount_pending_delivery())
            else:
                result = super(
                    SaleOrderLine, line)._compute_qty_amount_pending_delivery()
        return result
        ...
        for repair in self.repair_order_ids.filtered(
            ...
            if not repair.from_repair_picking_out_id:
                ...
                move_lines = self.repair_picking_move_line_ids.filtered(
                    lambda x: x.lot_id == repair.lot_id and
                    x.picking_id == repair.from_repair_picking_out_id)
                if not move_lines:
                    amount_pending_delivery += repair.amount_untaxed
                else:
    ...
    def _get_info_shipped_pending_invoicing(self):
        ...
        qty = 0
        for repair in self.repair_order_ids.filtered(
            lambda x: x.state != "cancel" and x.from_repair_picking_out_id and
                x.invoice_method != "none" and not x.invoice_id):
            move_lines = self.repair_picking_move_line_ids.filtered(
                lambda z: z.picking_id == repair.from_repair_picking_out_id and
                z.sale_line_id == repair.sale_line_id and
                z.lot_id == repair.lot_id and z.state == "done")
            if move_lines:
    ...
    @api.depends("product_uom_qty", "qty_invoiced", "discount", "price_unit",
                 "repair_order_ids", "repair_order_ids.amount_untaxed",
                 "repair_order_ids.invoice_method", "repair_order_ids.state",
                 "repair_order_ids.invoice_id",
    ...
    def _compute_qty_delivered(self):
        ...
        for line in self.filtered(lambda x: x.is_service and x.is_repair and
                                  x.product_to_repair_id):
            line.write({'service_repair_product_id': line.product_id.id,
