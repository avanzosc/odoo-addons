...
class AccountMoveLine(models.Model):
    ...
    estimate_hour = fields.Float(
        string="Estimated Hours", compute="_compute_estimate_hour", store=True
    )
    ...
    quantity2 = fields.Float(
        ...
        compute="_compute_estimate_hour",
        store=True,
    )
    calculated_quantity2 = fields.Boolean(
        string="Calculated_quantity2", compute="_compute_estimate_hour", store=True
    )

    @api.depends(
        "move_id",
        "move_id.timesheet_ids",
        "move_id.timesheet_ids.time_type_id",
        "move_id.timesheet_ids.unit_amount",
        "move_id.timesheet_ids.amount",
        "move_id.timesheet_ids.so_line",
        "move_id.start_date_period",
        "move_id.end_date_period",
        "sale_line_ids",
    )
    def _compute_estimate_hour(self):
        ...
        for line in self:
            ...
            if not line.contract_line_id:
                ...
                if (line.move_id.start_date_period) and (line.move_id.end_date_period):
                    timesheets = timesheets.filtered(
                        lambda x: x.date >= line.move_id.start_date_period
                        and (x.date <= line.move_id.end_date_period)
                    )
                ...
                line.limit_hour = 1 * (line.estimate_hour - line.hour_type4)
                ...
                if (
                    "<NewId" not in str(line)
                    and line.quantity2 > 0
                    and line.quantity != line.quantity2
                ):
                    self.env.cr.execute(
