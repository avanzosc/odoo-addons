...
class ResPartner(models.Model):
    ...
    def get_partner_phase_slides(self, channel):
        ...
        if channel.content_view == "phase" and channel.sudo().channel_partner_ids:
            ...
            domain = [("channel_id", "=", channel.id), ("is_preview", "!=", True)]
            ...
            show_slide_ids = (
                ...
                .sudo()
                .search(domain, order="sequence", limit=limit)
            )

            if show_slide_ids:
                next_slide = self.get_partner_next_slide(show_slide_ids[-1], channel)
                if next_slide:
    ...
    def get_partner_next_slide(self, slide_id, channel):
        ...
        partner_slide_info_id = channel.sudo().slide_partner_ids.filtered(
            lambda i: i.partner_id.id == self.id and i.slide_id.id == slide_id.id
        )
