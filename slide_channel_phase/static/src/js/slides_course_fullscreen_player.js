  ...
  var Sidebar = publicWidget.Widget.extend({
    ...
    setSlideCompleted: function (slideId) {
      ...
      $elem
        ...
        .addClass("fa-check text-success o_wslides_slide_completed");
      location.reload();
    },
    /**
        ...
        this._rpc({
          ...
          form.html(
            '<div class="alert alert-info" role="alert">' +
              _t("<strong>Thank you!</strong> Mail has been sent.") +
              "</div>"
          );
        });
      } else {
    ...
    _preprocessSlideData: function (slidesDataList) {
      slidesDataList.forEach(function (slideData, index) {
        ...
        if (slideData.type === "video") {
          ...
          slideData.embedUrl = _.str.sprintf(
            "/web/image/slide.slide/%s/image_1024",
            slideData.id
          );
        ...
        slideData.isQuiz = Boolean(slideData.isQuiz);
        slideData.hasQuestion = Boolean(slideData.hasQuestion);
        // Technical settings for the Fullscreen to work
        slideData._autoSetDone =
          _.contains(
            ["infographic", "presentation", "document", "webpage"],
            slideData.type
          ) && !slideData.hasQuestion;
      });
          ...
          if (slide._autoSetDone && !session.is_website_user) {
            ...
            if (["document", "presentation"].includes(slide.type)) {
              ...
              self.el
                ...
                .addEventListener("load", () => self._setCompleted(slide.id));
            } else {
  ...
  publicWidget.registry.websiteSlidesFullscreenPlayer = publicWidget.Widget.extend({
    ...
    start: function () {
      ...
      return Promise.all(proms).then(function () {
        $('#edit-page-menu a[data-action="edit"]').on(
          "click",
          self._onWebEditorClick.bind(self)
        );
      });
    },
