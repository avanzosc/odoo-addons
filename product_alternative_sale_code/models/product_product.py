...
class ProductProduct(models.Model):
    ...
    alternative_sales_code = fields.Char(
        string='Alternative sales code', copy=False)
        ...
        for record in self:
            if record.default_code:
                ...
                if product:
                    raise ValidationError(
                        _('Internal reference used in product: {}').format(
                            product.name))
            if record.barcode:
                cond = [('id', '!=', record.id),
            ...
            if record.alternative_sales_code:
                cond = [
                    ...
                    ('alternative_sales_code', '=',
                     record.alternative_sales_code)]
                product = self.search(cond, limit=1)
                if product:
                    raise ValidationError(
            ...
            for product in self:
                if product.default_code and not product.barcode:
                    product.with_context(
                        from_my_write=True).barcode = product.default_code
                if product.barcode and not product.default_code:
                    product.with_context(
                        from_my_write=True).default_code = product.barcode
