    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        for line in batches:
            ...
            chick_lines = line.move_line_ids.filtered(
                lambda c: c.move_type_id == chick_type and c.picking_id and (
                    c.date.date() <= date
                )
            )
            ...
            meat_units = sum(
                output_meat_lines.mapped("download_unit")
            ) - sum(
            ...
            chick_amount = sum(
                entry_chick_lines.mapped("amount")
            ) - sum(
            ...
            quant_amount = 0
            for lot in dif_lots:
                quant_amount += sum(stock_feed.filtered(lambda c: c.lot_id == lot).mapped("value"))
            ...
            feed_amount = sum(
                entry_feed_lines.mapped("amount")
            ) - sum(
                dev_feed_lines.mapped("amount")
            ) + quant_amount
            worksheet.write(n, 8, feed_amount, two_decimal_format)
            medicine_lines = line.move_line_ids.filtered(
                lambda c: c.move_type_id == medicine_type and (
                    c.picking_id
            ...
            medicine_amount = sum(
                entry_medicine_lines.mapped("amount")
            ) - sum(
            ...
            meat_qty_done = sum(
                output_meat_lines.mapped("qty_done")
            ) - sum(
