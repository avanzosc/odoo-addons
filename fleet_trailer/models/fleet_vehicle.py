...
class FleetVehicle(models.Model):
    ...
    options_included = fields.Text(
        string='Options included in the type approval')
    reforms_in_vehicle = fields.Text(
        string='Reforms in vehicle')
    ...
    password_for_total = fields.Char(
        string='(K) - Password')
    total_tare = fields.Char(
        string='Total tare')
    tare_on_axes = fields.Char(
    ...
    total_width = fields.Char(
        string='(F.5) - Total width')
    total_lenght = fields.Char(
        string='(F.6) - Total lenght')
    previous_way = fields.Char(
        string='(F.7) - Previous way')
    posterior_pathway = fields.Char(
        string='(F.7.1) - Posterior pathway')
    posterior_overhang = fields.Char(
        string='(F.8) - Posterior overhang')
    distance_between_axis = fields.Char(
        string='(M.1) - Distance between axis')
    number_of_axes = fields.Char(
        string='(L) - Number of axes')
    total_wheels_number = fields.Char(
    ...
    f11 = fields.Char(
        string='F11', compute='_compute_f11')
    f12 = fields.Char(
        string='F12', compute='_compute_f12')
    l = fields.Char(
        string='L', compute='_compute_l')
    l2 = fields.Char(
        string='L2', compute='_compute_l2')
    d2 = fields.Char(
        string='D2', compute='_compute_d2')

    @api.model
    def create(self, vals):
        vals["sequence"] = self.env.ref("fleet_trailer.seq_fleet_vehicle",
    ...
    def _compute_f11(self):
        for vehicle in self:
            ...
            if vehicle.vehicle_mmta_axe2_id:
                f11 = (vehicle.vehicle_mmta_axe2_id.name if not f11 else
                       "{} // {}".format(
                           f11, vehicle.vehicle_mmta_axe2_id.name))
            if vehicle.vehicle_mmta_axe3_id:
                f11 = (vehicle.vehicle_mmta_axe3_id.name if not f11 else
                       "{} // {}".format(
                           f11, vehicle.vehicle_mmta_axe3_id.name))
            if vehicle.vehicle_mmta_axe4_id:
                f11 = (vehicle.vehicle_mmta_axe4_id.name if not f11 else
                       "{} // {}".format(
                           f11, vehicle.vehicle_mmta_axe4_id.name))
            self.f11 = f11
    ...
    def _compute_f12(self):
        for vehicle in self:
            ...
            if vehicle.vehicle_mma_axe2_id:
                f12 = (vehicle.vehicle_mma_axe2_id.name if not f12 else
                       "{} // {}".format(
                           f12, vehicle.vehicle_mma_axe2_id.name))
            if vehicle.vehicle_mma_axe3_id:
                f12 = (vehicle.vehicle_mma_axe3_id.name if not f12 else
                       "{} // {}".format(
                           f12, vehicle.vehicle_mma_axe3_id.name))
            if vehicle.vehicle_mma_axe4_id:
                f12 = (vehicle.vehicle_mma_axe4_id.name if not f12 else
                       "{} // {}".format(
                           f12, vehicle.vehicle_mma_axe4_id.name))
            self.f12 = f12
    ...
    def _compute_l(self):
        for vehicle in self:
            ...
            if vehicle.total_wheels_number:
                l = (vehicle.total_wheels_number if not l else
                     "{} // {}".format(l, vehicle.total_wheels_number))
            self.l = l
    ...
    def _compute_l2(self):
        for vehicle in self:
            ...
            if vehicle.ic_iv_tire_id:
                l2 = (vehicle.ic_iv_tire_id.name if not l2 else
                      "{}    {}".format(l2, vehicle.ic_iv_tire_id.name))
            self.l2 = l2
    ...
    def _compute_d2(self):
        for vehicle in self:
            ...
            if vehicle.vehicle_variant_id:
                d2 = (vehicle.vehicle_variant_id.name if not d2 else
                      "{}/{}".format(d2, vehicle.vehicle_variant_id.name))
            if vehicle.vehicle_version_id:
                d2 = (vehicle.vehicle_version_id.name if not d2 else
                      "{}/{}".format(d2, vehicle.vehicle_version_id.name))
            self.d2 = d2
