...
class StockMoveLine(models.Model):
    ...
    product_brand_code = fields.Char(
        string="Product Brand Code", compute="_compute_brand_info",
        store=True, copy=False
    )
    brand_fabricators = fields.Char(
        string="Brand Fabricators", compute="_compute_brand_info",
        store=True, copy=False
    )
        ...
        for move in self:
            ...
            if move.product_id:
                ...
                for seller in sellers:
                    ...
                    product_brand_code = (
                        brand_code if not product_brand_code else
                        u"{}, {}".format(product_brand_code, brand_code))
                    partner_name = seller.partner_id.name
                    brand_fabricators = (
                        partner_name if not brand_fabricators else
                        u"{}, {}".format(brand_fabricators, partner_name))
    ...
    def _get_aggregated_product_quantities(self, **kwargs):
        ...
        in_picking_lines = self.filtered(
            ...
            for move_line in self:
                ...
                if line_key in clave:
                    result[clave]["fabricator"] = (
                        "" if not move_line.brand_fabricators else
                        move_line.brand_fabricators)
                    result[clave]["brand_code"] = (
                        "" if not move_line.product_brand_code else
                        move_line.product_brand_code)
        return result
