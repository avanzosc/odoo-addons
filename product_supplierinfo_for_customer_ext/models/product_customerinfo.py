...
class ProductCustomerInfo(models.Model):
    ...
    def create(self, vals):
        if (
            ...
            and "model" in self.env.context.get("params", {})
            and self.env.context.get("params").get("model", "aa") == "product.product"
        ):
        ...
        customer_info = super().create(vals)
        ...
        if "product_id" not in vals and "product_tmpl_id" not in vals:
            for info in self:
                ...
                if info.product_id and not info.product_tmpl_id:
                    my_vals["product_tmpl_id"] = info.product_id.product_tmpl_id.id
    ...
    def ir_cron_fix_product_producttmpl_in_product_customerinfo(self):
        ...
        cond = [("product_id", "!=", False), ("product_tmpl_id", "=", False)]
        ...
        for info in infos:
            if len(info.product_tmpl_id.product_variant_ids) == 1:
                info.product_id = info.product_tmpl_id.product_variant_ids[0].id
            if len(info.product_tmpl_id.product_variant_ids) > 1 and info.product_name:
