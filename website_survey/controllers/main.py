...
class CustomerPortal(CustomerPortal):
    def _prepare_portal_layout_values(self):
        ...
        values.update({"surveys_count": surveys_count})
        return values
    ...
    def survey_inputs(self, **post):
        ...
        return http.request.render("website_survey.portal_my_survey_inputs", values)

    @http.route(
        '/my/survey/<model("survey.user_input"):survey_input>',
        type="http",
        auth="user",
        website=True,
    )
    def survey_input(self, survey_input, **post):
        ...
        return http.request.render("website_survey.website_survey_input", values)


class Survey(Survey):
    @http.route(
        "/survey/certification/print/<string:survey_token>",
        type="http",
        auth="public",
        website=True,
        sitemap=False,
    )
    def certification_print(
        self, survey_token, review=False, answer_token=None, **post
    ):
