...
class EventTrack(models.Model):
    ...
    analytic_account_id = fields.Many2one(
        ...
        related="event_id.analytic_account_id",
        store=True,
    )
    ...
    def _catch_values_for_create_analytic_line(self, partner):
        name = "{} {} {}".format(self.event_id.name, self.name, self.date)
        analytic_line_vals = {
            "date": self.date,
            ...
            analytic_line_vals.update(
                {
                    ...
                    "product_uom_id": self.task_id.sale_line_id.product_id.uom_id.id,
                }
            )
        ...
        if partner:
            ...
            if not user:
                raise UserError(
                    _("User not found for speaker: {}").format(self.partner_id.name)
                )
            ...
            analytic_line_vals.update(
                {"partner_id": partner.id, "employee_id": employee.id}
            )
