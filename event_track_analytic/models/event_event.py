...
class EventEvent(models.Model):
    ...
    task_id = fields.Many2one(string="Task", comodel_name="project.task", copy=False)
    ...
    def write(self, vals):
        ...
        if "stage_id" in vals and vals.get("stage_id", False) == confirmed_stage.id:
    ...
    def search_project_in_sale_line(self):
        for ticket in self.event_ticket_ids:
            ...
            if sale_line and len(sale_line) == 1:
                ...
                if (
                    not sale_line.task_id
                    and sale_line.is_service
                    and sale_line.product_id.sale_ok
                    and sale_line.product_id.service_tracking
    ...
    def _onchange_project_id(self):
        if "no_update_project" not in self.env.context and self.project_id:
            self.with_context(no_update_name=True).name = self.project_id.name

    @api.onchange("name")
    def _onchange_name(self):
        if "no_update_name" not in self.env.context and self.name and self.project_id:
            self.project_id.with_context(no_update_project=True).name = self.name
    ...
    def button_open_analytic_lines(self):
        ...
        domain = expression.AND(
            [[("event_id", "in", self.ids)], safe_eval(action.domain or "[]")]
        )
