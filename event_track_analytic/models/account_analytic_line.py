...
class AccountAnalyticLine(models.Model):
    ...
    @api.depends("task_id", "task_id.project_id", "event_id", "event_id.project_id")
    def _compute_project_id(self):
        result = super()._compute_project_id()
        for line in self.filtered(lambda line: line.event_id):
            ...
            if line.event_id.task_id:
                line.task_id = line.event_id.task_id.id
        return result

    @api.model
    def create(self, values):
        if "event_track_id" in values and values.get("event_track_id", False):
            track = self.env["event.track"].browse(values.get("event_track_id"))
            values["event_id"] = track.event_id.id
        return super().create(values)

    def write(self, vals):
        if "event_track_id" in vals and vals.get("event_track_id", False):
            track = self.env["event.track"].browse(vals.get("event_track_id"))
