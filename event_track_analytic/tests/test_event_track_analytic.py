...
class TestEventTrackAnalytic(common.SavepointCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        vals = {
            "name": "User event track analytic",
            "login": "usereventrackanalytic@avanzosc.es",
        }
        cls.user = cls.env["res.users"].create(vals)
        vals = {"name": "Employee Event Track Analytic", "user_id": cls.user.id}
    ...
    def test_event_create_track(self):
        ...
        self.assertEqual(len(self.event.track_ids[0].account_analytic_line_ids), 1)
