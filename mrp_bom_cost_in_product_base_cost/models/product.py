...
class ProductProduct(models.Model):
    ...
    def action_bom_cost(self):
        return super(ProductProduct, self.with_context(
            update_base_cost=True)).action_bom_cost()

    def button_bom_cost(self):
        self.ensure_one()
        return super(ProductProduct, self.with_context(
            update_base_cost=True)).button_bom_cost()

    @api.multi
    def do_change_standard_price(self, new_price, account_id):
        result = super(ProductProduct, self.with_context(
            update_base_cost=True)).do_change_standard_price(
                new_price, account_id)
        return result
