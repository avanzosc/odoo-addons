...
class StockMoveLine(models.Model):
    ...
    def _onchange_qty_done(self):
        ...
        stock_qty = False
        if self.product_id and self.product_id.tracking == "none" and (
            self.location_id) and (
                self.location_id.usage == "internal"):
            stock_qty = self.env["stock.quant"].search([
                ("product_id", "=", self.product_id.id),
                ("location_id", "=", self.location_id.id)]).quantity
        elif self.product_id and self.product_id.tracking != "none" and (
            self.lot_id) and self.location_id and (
                self.location_id.usage == "internal"):
            stock_qty = self.env["stock.quant"].search([
                ("product_id", "=", self.product_id.id),
                ("location_id", "=", self.location_id.id),
                ("lot_id", "=", self.lot_id.id)]).quantity
