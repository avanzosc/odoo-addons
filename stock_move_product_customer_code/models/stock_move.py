...
class StockMove(models.Model):
    ...
    def _compute_customer_product_code(self):
        for move in self:
            product_code = ""
            if (move.product_id and move.product_id.customer_code_ids and
                    move.picking_id.partner_id):
                partner = (
                    move.picking_id.partner_id if not
                    move.picking_id.partner_id.parent_id else
                    move.picking_id.partner_id.parent_id)
    ...
    def _compute_product_display_name(self):
        for move in self:
            ...
            if (move.picking_id.picking_type_id.code == "outgoing" and
                move.product_id.default_code and
                move.customer_product_code and
                    move.product_id.default_code in product_display_name):
                new_value = product_display_name.replace(
                    move.product_id.default_code,
                    move.customer_product_code)
