...
class WebsiteSlides(WebsiteProfile):
    ...
    def slide_view(self, slide, **kwargs):
        res = super().slide_view(slide, **kwargs)
        ...
        res.qcontext.update({"keep": keep, "partner_slide": slide_channel_partner})
        return res

    @http.route(
        ["/slides/save_attachment"],
        type="json",
        auth="public",
        methods=["POST"],
        website=True,
    ...
    def slide_view_json(self, slide_id, attachment):
        ...
        slide_channel_partner = slide_slide_partner_obj.sudo().search(
            [("slide_id", "=", slide.id), ("partner_id", "=", logged_partner.id)]
        )
