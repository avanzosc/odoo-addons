...
===============================
Pricelist Several Prices Import
===============================

This module imports several prices into Odoo sale pricelists. It allows
each pricelist to be linked to a market, such as Spain, UK, or ROW, so
...
* Create and assign markets to pricelists.
...
* **Import**  prices from an ``.xlsx`` file using market columns.
...
2. Create the markets used in the spreadsheet, for example ``Spain``,
   ``UK`` and ``ROW``.
...
6. Upload the ``.xlsx`` file and optionally select **Catalogs**. Click
