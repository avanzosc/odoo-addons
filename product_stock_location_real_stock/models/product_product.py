...
class ProductProduct(models.Model):
    ...
    location_qty_available = fields.Float(
        ...
        compute="_compute_location_quantities", compute_sudo=False,
    ...
    def _compute_location_quantities(self):
        ...
        res = products.with_context(
            location_qty_available=True)._compute_quantities_dict(
    ...
    def action_open_quants_real_stock(self):
        action = self.with_context(
            search_default_realstock=True).action_open_quants()
        return action
    ...
    def _get_domain_locations(self):
        ...
        if "location_qty_available" in self.env.context:
            domain_move_in_loc = [
                ("location_dest_id.real_stock_location", "=", True)] + domain_move_in_loc
            domain_move_out_loc = [
                ("location_id.real_stock_location", "=", True)] + domain_move_out_loc
            domain_quant_loc = [
                ("real_stock_location", "=", True)] + domain_quant_loc
