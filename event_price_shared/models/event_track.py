...
class EventTrack(models.Model):
    ...
    shared_price_event = fields.Boolean(
        string="Shared price event", related="event_id.shared_price_event", store=True
    )

    def _catch_values_for_create_analytic_line(self, partner):
        values = super()._catch_values_for_create_analytic_line(partner)
        ...
        for task in tasks:
            task_registrations = registrations.filtered(lambda x: x.task_id == task)
            values["task_id"] = task.id
            if task.sale_line_id:
                values.update(
                    {
                        "product_id": task.sale_line_id.product_id.id,
                        "product_uom_id": task.sale_line_id.product_id.uom_id.id,
                    }
                )
                ...
                values.update(
                    {
                        ...
                        "product_uom_id": task.project_id.timesheet_product_i.uom_id.id,
                    }
                )
            amount = (self.duration / len(registrations)) * len(task_registrations)
