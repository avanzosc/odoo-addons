...
class ResPartnerAcademicBackground(models.Model):
    ...
    def _compute_count_phone_call(self):
        for call in self:
            call.count_phone_call = len(call.phone_call_ids)

    phone_call_ids = fields.One2many(
        string="Phone call",
        inverse_name="academic_background_id",
    ...
    linguistic_model = fields.Selection(
        [("a", "A"), ("b", "B"), ("d", "D"), ("t", "T")], string="Linguistic model"
    )
    tutor_id = fields.Many2one(string="Tutor", comodel_name="res.partner")
    tutor_phone = fields.Char(
        string="Tutor phone",
        comodel_name="res.partner",
        related="tutor_id.phone",
        store=True,
    )
    substitute_id = fields.Many2one(string="Substitute", comodel_name="res.partner")
    counselor_id = fields.Many2one(string="Counselor", comodel_name="res.partner")
    another_professional_ids = fields.One2many(
        comodel_name="res.partner.another.professional",
        inverse_name="academic_background_id",
        string="Another professional",
    )
        ...
        for academic_background in self:
            ...
            if academic_background.linguistic_model:
                ...
                literal = field.convert_to_export(
                    academic_background["linguistic_model"], academic_background
                )
            name = "{} {} {}".format(
