...
class AccountMoveImport(models.Model):
    ...
    def button_open_account_move(self):
        ...
        domain = expression.AND(
            [[("id", "=", account_moves.ids)], safe_eval(action.domain or "[]")]
        )
...
class AccountMoveImportLine(models.Model):
            ...
            if not account_move:
                ...
                if self.account_move_ref:
                    ...
                    same_ref_moves = self.import_id.import_line_ids.filtered(
                        ...
                        and (c.date == self.date)
                    )
                if same_ref_moves:
                    for line in same_ref_moves:
                        line.account_move_id = account_move.id
                    if not all([line.state == "pass" for line in same_ref_moves]):
                        log_info = _(
                            "There are lines with the same reference " + "with errors."
                        )
    ...
    def _check_account_move_ref(self):
        ...
        if self.account_move_ref:
            ...
            search_domain = [("ref", "=", self.account_move_ref)]
            if self.import_id.company_id:
                search_domain = expression.AND(
                    [
                        [
                ...
                account_move_obj = account_move_obj.with_company(
                    self.import_id.company_id
                )
            same_ref = account_move_obj.search(search_domain)
            if same_ref:
                log_info = _("Error: There is another invoice with this reference.")
        return log_info
    ...
    def _check_partner(self):
        ...
        if self.partner_code:
            search_domain = expression.AND(
                [[("ref", "=", self.partner_code)], search_domain]
            )
    ...
    def _check_product(self):
        ...
        if self.product_code:
            search_domain = expression.AND(
                [[("default_code", "=ilike", self.product_code)], search_domain]
            )
    ...
    def _create_account_move_line(self):
        ...
        if lines:
            for line in lines:
                ...
                if taxes and self.account_move_id.fiscal_position_id:
                    taxes = self.account_move_id.fiscal_position_id.map_tax(
    ...
    def _account_move_line_vals(self):
        ...
        if not account:
            if self.product_id and self.product_id.property_account_income_id:
                ...
                and (self.product_id.categ_id.property_account_income_categ_id)
            ):
        ...
        if self.account_move_id and (self.account_move_id.move_type == "out_refund"):
            if quantity < 0:
                quantity = -quantity
            else:
        ...
        vals = {
            ...
            "product_uom_id": self.product_id.uom_id and (self.product_id.uom_id.id),
    ...
    def _account_move_vals(self):
        ...
        vals = {
            ...
            "ref": self.account_move_ref,
            "date": self.date,
            "start_date_period": self.start_date_period,
            "end_date_period": self.end_date_period,
            "journal_id": self.import_id.journal_id and (self.import_id.journal_id.id),
            "company_id": self.import_id.company_id and (self.import_id.company_id.id),
        }
        return vals
