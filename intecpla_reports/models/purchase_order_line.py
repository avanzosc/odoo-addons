...
class PurchaseOrderLine(models.Model):
            ...
            if not supplier_info:
                supplier_info = line.product_id.seller_ids.filtered(
                    lambda s: (
                        ...
                        and s.name == line.partner_id and s.product_code))
                if supplier_info:
                    line.my_product_code = supplier_info[0].product_code
                else:
        ...
        for line in self:
            ...
            if not supplier_info:
                supplier_info = line.product_id.seller_ids.filtered(
                    lambda s: (
                        s.product_tmpl_id == line.product_id.product_tmpl_id
                        and s.name == line.partner_id and s.product_name))
                if supplier_info:
                    line.description_to_print = supplier_info[0].product_name
                else:
    ...
    my_product_code = fields.Char(
        string='Code', compute='_compute_my_product_code')
