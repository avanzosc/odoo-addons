        ...
        for line in self.import_line_ids.filtered(
            ...
            if line.account_asset_id and line.date and line.amount:
                ...
                if same_asset_lines:
                    ...
                    if (
                        sum(same_asset_lines.mapped("amount"))
                        != line.account_asset_id.depreciation_base
                    ):
                        log_info = _(
                            "The sum of the amount of the same asset has to be the same as the depreciation base amount."
                        )
            if log_info:
        ...
        if self.action == "create" and self.state not in ("error", "done"):
            ...
            if self.account_asset_id and self.date and self.amount:
                ...
                if same_asset_lines:
                    ...
                    if (
                        sum(same_asset_lines.mapped("amount"))
                        != self.account_asset_id.depreciation_base
                    ):
                        log_info = _(
                            "The sum of the amount of the same asset has to be the same as the depreciation base amount."
                        )
