...
class PurchaseOrderImport(models.Model):
    ...
    def _get_line_values(self, row_values=False):
        ...
        if row_values:
            ...
            purchase_date_planned = timezone.localize(
                purchase_date_planned).astimezone(pytz.UTC)
            ...
            values.update(
                {
                    "purchase_supplier_code": convert2str(
                    ...
                    "purchase_product_code": convert2str(
                        purchase_product_code),
                    "purchase_product_name": convert2str(
...
class PurchaseOrderImportLine(models.Model):
    ...
    def action_process(self):
        ...
        for line in self.filtered(lambda l: l.state not in ("error", "done")):
            if line.action == "create":
                ...
                if line.purchase_origin and (
                    line.purchase_origin) not in (
                        origins):
                    if self.filtered(lambda l: l.purchase_origin == (
                        line.purchase_origin) and (
                            l.state == "error")):
                        log_info = _(
                        ...
                        if not log_info:
                            ...
                            same_origin = self.filtered(
                                lambda l: l.purchase_origin == (
                                    line.purchase_origin))
    ...
    def _check_product(self):
        ...
        if not products:
            ...
            log_info = _(
                ...
                "code found.").format(self.purchase_product_name,
                                      self.purchase_product_code)
    ...
    def _check_warehouse(self):
        ...
        if not warehouses:
            ...
            warehouses = False
            log_info = _(
                "Error: More than one warehouse with name {} found."
                ).format(self.purchase_warehouse)
