...
class SlideChannelTag(models.Model):
    ...
    courses_count = fields.Integer(
        string="Courses number", compute="_compute_courses_count", store=True
    )
    ...
    def action_duplicate(self):
        default = {"name": "{} {}".format(self.name, _("(COPY)"))}
        ...
        action = self.env.ref("website_slides.slide_channel_tag_action").read()[0]
        return action
