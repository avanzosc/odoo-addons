...
class SlideChannelTagCourse(models.Model):
    ...
    tag_group_sequence = fields.Integer(
        "Group sequence", related="slide_channel_tag_id.group_sequence", store=True
    )
    slide_channel_tag_id = fields.Many2one(
        string="Slide channel tag", comodel_name="slide.channel.tag"
    )
    sequence = fields.Integer(string="Sequence", default=10)
    slide_channel_id = fields.Many2one(string="Course", comodel_name="slide.channel")
    ...
    technology_id = fields.Many2one(
        string="Technology",
        comodel_name="slide.channel.technology",
        related="slide_channel_id.technology_id",
        store=True,
    )
    technology_description = fields.Text(
        string="Technology description",
        store=True,
